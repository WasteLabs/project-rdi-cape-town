...
import mcarptif.solver.LS_MCARPTIF as LS
from mcarptif.solver.py_solution_test import TestCLARPIFSolution
from mcarptif.converter.load_data import load_instance
from mcarptif.converter.load_data import ConvertedInputs
from mcarptif.converter.solution_converter import convert_df
from mcarptif.converter.solution_converter import convert_df_full
from mcarptif.converter.solution_converter import write_solution_df
...
def initiate_local_search(info, test_solution=False, nnFrac=1):
    ...
    """
    improve = LS.LS_MCARPTIF(
        info, info.nn_list, testAll=test_solution, autoInvCosts=False
    )
...
def initiate_tabu_search(info, test_solution, nnFrac=1):
    ...
    improve.setTabuSearchParameters(
        compoundMoves,
        newCompoundMoves,
        tabuTenure,
        tabuThreshold,
...
def solve_instance(
    ...
    solution = gen_solution(info, reduce_initial_trips, test_solution=test_solution)
...
9                7327           25941        2673      2859
    ...
    solution = gen_solution(
        info, reduce_initial_trips, test_solution=debug_test_solution
    )
...
def build_tweak_solution(solution, full_output=True, info=None):
