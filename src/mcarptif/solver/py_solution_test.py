    ...
    def testroutecosts(self, route_solution, total_cost):
        ...
        if route_solution["Cost"] != cost:
            ...
            if self.print_output:
                ...
                logging.error(
                    "Incorrect route cost - Actual: %i \t Specified: %i"
                    % (cost, route_solution["Cost"])
                )
        if route_solution["Deadhead"] != deadhead and self.deadheading:
            ...
            if self.print_output:
                logging.error("Cost Error")
                logging.error(
                    "Incorrect route deadheading cost - Actual: %i \t Specified: %i"
                    % (deadhead, route_solution["Deadhead"])
                )
        if route_solution["Service"] != servicecost and self.deadheading:
            ...
            if self.print_output:
                logging.error("Cost Error")
                logging.error(
                    "Incorrect route service cost - Actual: %i \t Specified: %i"
                    % (servicecost, route_solution["Service"])
                )
        if route_solution["Load"] != load:
            ...
            if self.print_output:
                logging.error("Cost Error")
                logging.error(
                    "Incorrect route load - Actual: %i \t Specified: %i"
                    % (load, route_solution["Load"])
                )
        if route_solution["Load"] > self.info.capacity:
            ...
            if self.print_output:
                ...
                logging.error(
                    "Load is more than vehicle capacity - Actual: %i \t Limit: %i"
                    % (route_solution["Load"], self.info.capacity)
                )
    ...
    def testCARPsolution(self):
        ...
        if nRoutes != len(self.solution) - 3:
            ...
            if self.print_output:
                ...
                logging.error(
                    "More vehicles than specified - Actual: %i \t Specified: %i"
                    % (nRoutes, len(self.solution) - 3)
                )
        ...
        if total_cost != self.solution["TotalCost"]:
            ...
            if self.print_output:
                logging.error("Cost Error")
                logging.error(
                    "Incorrect solution cost - Actual: %i \t Specified: %i"
                    % (total_cost, self.solution["TotalCost"])
                )
...
class TestCLARPIFSolution(object):
    ...
    def testtripcosts(self, trip, route_cost_info):
        (trip_cost, trip_dead_head_cost, trip_service_cost, trip_load) = route_cost_info
        cost = 0
        deadhead = 0
        servicecost = 0
        load = 0
        ...
        if not cost - self.tollerance <= trip_cost <= cost + self.tollerance:
            ...
            logging.error("Cost Error")
            logging.error(
                "Incorrect trip cost - Actual: %i \t Specified: %i" % (cost, trip_cost)
            )
            raise ValueError
        if (
            not deadhead - self.tollerance
            <= trip_dead_head_cost
            <= deadhead + self.tollerance
        ):
            self.Error = self.costError = True
            logging.error("Cost Error")
            logging.error(
                "Incorrect trip deadheading cost - Actual: %i \t Specified: %i"
                % (trip_dead_head_cost, deadhead)
            )
            raise ValueError
        if (
            not servicecost - self.tollerance
            <= trip_service_cost
            <= servicecost + self.tollerance
        ):
            self.Error = self.costError = True
            logging.error("Cost Error")
            logging.error(
                "Incorrect trip service cost - Actual: %i \t Specified: %i"
                % (trip_service_cost, servicecost)
            )
            raise ValueError
        ...
        if trip_load > self.info.capacity + self.tollerance:
            ...
            logging.error("Constraint Error")
            logging.error(
                "Load is more than vehicle capacity - Actual: %i \t Specified: %i"
                % (trip_load, self.info.capacity)
            )
            raise ValueError
    ...
    def testroute(self, route_solution, servicedarcs, total_cost):
        ...
        for i in range(nTrips):
            if (i == 0) & (i == (nTrips - 1)):
                ...
                (servicedarcs) = self.testservice(
                    route_solution["Trips"][i][:-1], servicedarcs
                )
                self.testIFdepot(route_solution["Trips"][i])
            elif i == 0:
                self.testDepot(route_solution["Trips"][i])
                (servicedarcs) = self.testservice(
                    route_solution["Trips"][i], servicedarcs
                )
            elif i == (nTrips - 1):
                (servicedarcs) = self.testservice(
                    route_solution["Trips"][i][:-1], servicedarcs
                ...
                self.testIFdepot(route_solution["Trips"][i])
                self.testIFs(
                    route_solution["Trips"][i][0], route_solution["Trips"][i - 1][-1]
                )
            else:
                (servicedarcs) = self.testservice(
                    route_solution["Trips"][i], servicedarcs
            ...
            route_cost_info = (
                route_solution["TripCosts"][i],
                route_solution["TripDeadheads"][i],
                route_solution["TripServices"][i],
                route_solution["TripLoads"][i],
            )
            print(f"Test trip {i}")
            (cost, deadhead, servicecost, load) = self.testtripcosts(
                route_solution["Trips"][i], route_cost_info
            )
            route_cost += cost
        ...
        if (
            not route_cost - self.tollerance
            <= route_solution["Cost"]
            <= route_cost + self.tollerance
        ):
            self.Error = self.costError = True
            logging.error("Cost Error")
            logging.error(
                "Incorrect route cost - Actual: %i \t Specified: %i"
                % (route_cost, route_solution["Cost"])
            )
            raise ValueError
        if (
            not route_deadhead - self.tollerance
            <= route_solution["Deadhead"]
            <= route_deadhead + self.tollerance
        ):
            self.Error = self.costError = True
            logging.error("Cost Error")
            logging.error(
                "Incorrect route deadhead cost - Actual: %i \t Specified: %i"
                % (route_deadhead, route_solution["Deadhead"])
            )
            raise ValueError

        if (
            not route_servicecost - self.tollerance
            <= route_solution["Service"]
            <= route_servicecost + self.tollerance
        ):
            self.Error = self.costError = True
            logging.error("Cost Error")
            logging.error(
                "Incorrect route service cost - Actual: %i \t Specified: %i"
                % (route_servicecost, route_solution["Service"])
            )
            raise ValueError
        if (
            not route_load - self.tollerance
            <= route_solution["Load"]
            ...
            logging.error(
                "Incorrect route load - Actual: %i \t Specified: %i"
                % (route_load, route_solution["Load"])
            )
            raise ValueError
        if route_cost > self.info.maxTrip + self.tollerance:
            ...
            logging.error("Constraint Error")
            logging.error(
                "Route cost is more than max route cost - Actual: %i \t Specified: %i"
                % (route_cost, self.info.maxTrip)
            )
            raise ValueError
    ...
    def testCLARPIFsolution(self):
        ...
        for i in range(nRoutes):
            ...
            (servicedarcs, total_cost) = self.testroute(
                self.solution[i], servicedarcs, total_cost
            )
        if servicedarcs:
            self.Error = self.serviceError = True
            logging.error("Service Error")
            logging.error("Arcs not serviced", servicedarcs)
        if (
            not self.solution["TotalCost"] - self.tollerance
            <= total_cost
