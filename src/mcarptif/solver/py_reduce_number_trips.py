...
from mcarptif.solver.py_solution_builders import build_CARP_list
from mcarptif.solver.py_solution_builders import build_CLARPIF_list
from mcarptif.solver.py_solution_builders import build_CARP_dict_from_list
from mcarptif.solver.py_solution_builders import build_CLARPIF_dict_from_list
from mcarptif.solver.py_display_solution import display_solution_stats
from mcarptif.solver.py_route_modifier import insert_multiple_arcs
from mcarptif.solver.py_solution_test import test_solution  # From Dev_SolutionOperators
...
class Reduce_Trips(object):
    ...
    def reduce_carp_routes(self, solution_lists, insert_depots=True):
        ...
        while reduction:
            ...
            (
                change,
                solution_lists_new,
            ) = self.insert_multiple_arcs.insert_multiple_arcs_carp(
                solution_lists_new, arcs_insert
            )
            for k in range(len(solution_lists_new[0])):
                del solution_lists_new[0][k][0]
                del solution_lists_new[0][k][-1]
            if change:
                return (change, solution_lists_new)
            else:
                reduction -= 1  # return(change, solution_lists)
    ...
    def reduce_clarpif_routes(self, solution_lists, nRemove=1):
        ...
        if (t_service < excess_cap) and (nRemove < nRoutes):
            ...
            (
                change,
                solution_lists_new,
            ) = self.insert_multiple_arcs.insert_multiple_arcs_clarpif(
                solution_lists_new, arcs_insert
            )
            if change:
                return (change, solution_lists_new)
    ...
    def reduce_CARP_solution(self, solution):
        ...
        if change:
            ...
            solution = build_CARP_dict_from_list(
                new_routes, new_loads, new_service, new_deadhead, new_costs, self.depot
            )
    ...
    def reduce_CLARPIF_solution(self, solution):
        ...
        while True:
            n += 1
            (change, solution_list_best) = self.reduce_clarpif_routes(
                deepcopy(solution_list), n
            )
            if change:
        ...
        if reduced:
            ...
            solution = build_CLARPIF_dict_from_list(
                routes,
                load,
                service,
                deadhead,
    ...
    def reduce_CLARPIF_solution_v2(self, solution):
        ...
        while True:
            n = 1
            (change, solution_list_best) = self.reduce_clarpif_routes_v2(
                deepcopy(solution_list), n
            )
            if change:
    ...
    def reduce_CLARPIF_solution_v3(self, solution):
        ...
        while n < 2:
            n += 1
            (change, solution_list_best) = self.reduce_clarpif_routes_v3(
                deepcopy(solution_list), n
            )
            if change:
                solution_list_final = deepcopy(solution_list_best)
    ...
    def reduce_trip(self, solution, v2=True, v3=False):
        ...
        if solution["ProblemType"] == "CARP":
            ...
            if v2:
                (reduced, solution) = self.reduce_CLARPIF_solution_v2(solution)
            elif v3:
                (reduced, solution) = self.reduce_CLARPIF_solution_v3(solution)
            else:
                (reduced, solution) = self.reduce_CLARPIF_solution(solution)
        if reduced:
