...
import os
import mcarptif.solver.sysPath as sysPath
import sys

sys = sysPath.addInputPaths(sys)
mainPath = sysPath.returnPath()

import mcarptif.solver.calcMoveCost as calcMoveCost
import mcarptif.solver.py_display_solution as py_display_solution
from copy import deepcopy
from time import perf_counter as clock
from mcarptif.solver.py_reduce_number_trips import Reduce_Trips as ReduceRoutes
from mcarptif.solver.py_solution_builders import build_CLARPIF_dict_correct
...
class TestLocalSeach(object):
    ...
    def _testSolution(self, solution):
        ...
        for i in range(solution["nVehicles"]):
            ...
            for tripIndex, trip in enumerate(routeI):
                ...
                deadheadCost = (
                    sum([self._d[trip[j]][trip[j + 1]] for j in range(len(trip) - 1)])
                    + self._dumpCost
                )
                routeCost += serviceCost + deadheadCost
                if (
                    not demand - self.tollerance
                    <= solution[i]["TripLoads"][tripIndex]
                    <= demand + self.tollerance
                ):
                    errorsFound = True
                    print(
                        "ERROR: Incorrect route load specified for route %i trip %i: actual %i vs %i"
                        % (i, tripIndex, demand, solution[i]["TripLoads"][tripIndex])
                    )
                if demand > self._capacity + self.tollerance:
                    errorsFound = True
                    print(
                        "ERROR: Trip demand exceeds capacity for route %i trip %i: actual %i vs %i"
                        % (i, tripIndex, demand, self._capacity)
                    )
                if tripIndex == 0 and routeI[tripIndex][0] != self._depot:
                    errorsFound = True
                    print(
                        "ERROR: First arc not the depot in route %i: %i"
                        % (i, routeI[tripIndex][0])
                    )
                lastArcI = -1
                if tripIndex == nTrips - 1:
                    ...
                    if routeI[tripIndex][-1] != self._depot:
                        errorsFound = True
                        print(
                            "ERROR: Last arc not the depot in route %i: %i"
                            % (i, routeI[tripIndex][-1])
                        )
                    if routeI[tripIndex][-2] not in self._ifs:
                        errorsFound = True
                        print(
                            "ERROR: Second last arc not IF in route %i: %i"
                            % (i, routeI[tripIndex][-2])
                        )
                elif routeI[tripIndex][-1] not in self._ifs:
                    errorsFound = True
                    print(
                        "ERROR: last arc not IF in route %i trip %i: %i"
                        % (i, tripIndex, routeI[tripIndex][-1])
                    )
                for arc in routeI[tripIndex][1:lastArcI]:
                    if arc not in arcsNotService:
                        errorsFound = True
                        print(
                            "ERROR: Arc does not have to be serviced in route %i trip %i: %i"
                            % (i, tripIndex, arc)
                        )
                    else:
                ...
                if tripIndex < len(routeI) - 1:
                    bestIF = self._if_arc[routeI[tripIndex][-2]][
                        solution[i]["Trips"][tripIndex + 1][1]
                    ]
                    if routeI[tripIndex][-1] != bestIF:
                        errorsFound = True
                        print(
                            "ERROR: Suboptimal IF visit at the end of trip %i: %i instead of %i"
                            % (i, routeI[tripIndex][-1], bestIF)
                        )
                    if routeI[tripIndex][-1] != solution[i]["Trips"][tripIndex + 1][0]:
                        errorsFound = True
                        print(
                            "ERROR: Last IF visit at the end of trip %i not the same as next trip: %i and %i"
                            % (
                                i,
                                routeI[tripIndex][-1],
                                solution[i]["Trips"][tripIndex + 1][0],
                            )
                        )

            if (
                not routeCost - self.tollerance
                <= solution[i]["Cost"]
                <= routeCost + self.tollerance
            ):
                errorsFound = True
                print(
                    "ERROR: Incorrect route cost specified for route %i: actual %i vs %i"
                    % (i, routeCost, solution[i]["Cost"])
                )
                # totalCost += serviceCost + deadheadCost
            if routeCost > self._maxTrip + self.tollerance:
                errorsFound = True
                print(
                    "ERROR: Route cost exceeds max cost limit for route %i: actual %i vs %i"
                    % (i, routeCost, self._maxTrip)
                )
            totalCost += routeCost

        if (
            not totalCost - self.tollerance
            <= solution["TotalCost"]
            <= totalCost + self.tollerance
        ):
            errorsFound = True
            print(
                "ERROR: Incorrect total cost specified: actual %i vs %i"
        ...
        for arc in arcsNotService:
            ...
            if arc in self._edgesS:
                print(
                    "ERROR: Not all required arcs serviced: %i or %i"
                    % (arc, self._inv[arc])
                )
            else:
        ...
        if errorsFound:
            ...
            raise NameError(
                "Errors found with solution above, please see comments above solution."
            )
    ...
    def _testRelocateMove(self, moveInfo, solutionMapping, solution):
        ...
        (
            relocateCost,
            (arcI, preArcI, postArcI, arcJ, preArcJ, postArcJ),
            moveType,
            (moveCostsI, moveCostsJ),
        ) = moveInfo
        errorsFound = False
        ...
        if moveType.find("relocate") == -1:
            errorsFound = True
            print(
                "ERROR: Incorrect move test (%s) for this kind of move (relocate)."
                % moveType
            )
        ...
        if relocateCost != moveCostsI + moveCostsJ:
            errorsFound = True
            print(
                "ERROR: Cost change per route not the same as solution cost change: %i + %i != %i"
                % (moveCostsI, moveCostsJ, relocateCost)
            )
        ...
        if moveType.find("relocatePreArcIF") != -1:
            ...
            actualMoveCostsI = self._if_cost[preArc][postArc] - (
                self._d[preArc][arc] + self._if_cost[arc][postArc]
            )
            ...
            actualMoveCostsI = self._if_cost[preArc][postArc] - (
                self._if_cost[preArc][arc] + self._d[arc][postArc]
            )
        else:
            preArc = solution[routeI]["Trips"][tripI][removePos - 1]
            arc = solution[routeI]["Trips"][tripI][removePos]
            postArc = solution[routeI]["Trips"][tripI][removePos + 1]
            actualMoveCostsI = self._d[preArc][postArc] - (
                self._d[preArc][arc] + self._d[arc][postArc]
            )

        if arcI != arc and self._inv[arcI] != arc:
            errorsFound = True
            print(
                "ERROR: Arc to remove not the same as the one currently in the route position %i: actual %i vs %i"
                % (routeI, arc, arcI)
            )
        ...
        if preArc != preArcI:
            errorsFound = True
            print(
                "ERROR: Arc now before remove arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeI, preArc, preArcI)
            )
            print(solution[routeI]["Trips"][tripI])
        ...
        if postArc != postArcI:
            errorsFound = True
            print(
                "ERROR: Arc now after remove arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeI, postArc, postArcI)
            )
        ...
        if actualMoveCostsI != moveCostsI:
            errorsFound = True
            print(
                "ERROR: Calculated remove move cost not the same as given for route %i: actual %i vs %i"
                % (routeI, actualMoveCostsI, moveCostsI)
            )
            print(solution[routeI]["Trips"][tripI])
        ...
        actuaMoveCostsJ = (
            self._d[preArc][arcI] + self._d[arcI][arc] - self._d[preArc][arc]
        )

        if arcJ != arc:
            errorsFound = True
            print(
                "ERROR: Arc to insert at not the same as the one currently in the route position %i: actual %i vs %i"
                % (routeJ, arc, arcJ)
            )
        ...
        if preArc != preArcJ:
            errorsFound = True
            print(
                "ERROR: Arc now before insert at arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeJ, preArc, preArcJ)
            )
            print(solution[routeJ]["Trips"][tripJ])
        ...
        if actuaMoveCostsJ != moveCostsJ:
            errorsFound = True
            print(
                "ERROR: Calculated insert move cost not the same as given for route %i: actual %i vs %i"
                % (routeJ, actuaMoveCostsJ, moveCostsJ)
            )
            print(solution[routeJ]["Trips"][tripJ])
        ...
        if actualMoveCostsI + actuaMoveCostsJ != relocateCost:
            errorsFound = True
            print(
                "ERROR: Actual cost change per route not the same as solution cost change: %i + %i != %i"
                % (actualMoveCostsI, actuaMoveCostsJ, relocateCost)
            )
            print(solution[routeI]["Trips"][tripI])
            print(solution[routeJ]["Trips"][tripJ])
        ...
        if errorsFound:
            ...
            print(moveInfo)
            raise NameError(
                "Errors found with move to solution above, please see comments above solution."
            )
    ...
    def _testRelocateIFMove(self, moveInfo, solutionMapping, solution):
        ...
        if moveType.find("_PreIF") != -1:
            ...
            actuaMoveCostsJ = (
                self._d[preArc][arcI]
                + self._if_cost[arcI][arc]
                - self._if_cost[preArc][arc]
            )
            preArcJ = arcJ
            arcJ = postArcJ
    ...
    def _testDoubleRelocateMove(self, moveInfo, solutionMapping, solution):
        ...
        (
            relocateCost,
            (arcI1, preArcI, postArcI, arcJ, preArcJ, postArcJ),
            moveType,
            (moveCostsI, moveCostsJ),
        ) = moveInfo
        errorsFound = False
        ...
        postArc = solution[routeI]["Trips"][tripI][removePos + 2]
        actualMoveCostsI = self._d[preArc][postArc] - (
            self._d[preArc][arc1] + self._d[arc2][postArc]
        )

        if arcI1 != arc1 or arcI2 != arc2:
            errorsFound = True
            print(
                "ERROR: Arc to remove not the same as the one currently in the route position %i: actual %i, %i vs %i, %i"
                % (routeI, arc1, arcI1, arc2, arcI2)
            )
        ...
        arc = solution[routeJ]["Trips"][tripJ][insertPos]
        actuaMoveCostsJ = (
            self._d[preArc][arcI1] + self._d[arcI2][arc] - self._d[preArc][arc]
        )
    ...
    def _testExchangeMove(self, moveInfo, solutionMapping, solution):
        ...
        if moveType.find("1excPostIF") != -1:
            ...
            postArc = solution[routeI]["Trips"][tripI][removePos + 1]
            actualMoveCostsI = (
                self._if_cost[preArc][arcJ]
                + self._d[arcJ][postArc]
                - (self._if_cost[preArc][arc] + self._d[arc][postArc])
            )
        ...
        if arcI != arc and self._inv[arcI] != arc:
            errorsFound = True
            print(
                "ERROR: Arc to exchange not the same as the one currently in the route position %i: actual %i vs %i"
                % (routeI, arc, arcI)
            )
            print(solution[routeI]["Trips"])
        ...
        if preArc != preArcI:
            errorsFound = True
            print(
                "ERROR: Arc now before exchange arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeI, preArc, preArcI)
            )
            print(solution[routeI]["Trips"])
        ...
        if postArc != postArcI:
            errorsFound = True
            print(
                "ERROR: Arc now after exchange arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeI, postArc, postArcI)
            )
            print(solution[routeI]["Trips"])
        ...
        if actualMoveCostsI != moveCostsI:
            errorsFound = True
            print(
                "ERROR: Calculated exchange move cost not the same as given for route %i: actual %i vs %i"
                % (routeI, actualMoveCostsI, moveCostsI)
            )
            print(solution[routeI]["Trips"])
        ...
        if preArc != preArcJ:
            errorsFound = True
            print(
                "ERROR: Arc now before exchange not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeJ, preArc, preArcJ)
            )
            print(solution[routeJ]["Trips"])
        ...
        if postArc != postArcJ:
            errorsFound = True
            print(
                "ERROR: Arc now after exchange not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeJ, postArc, postArcJ)
            )
            print(solution[routeJ]["Trips"])
    ...
    def _testFlipMove(self, moveInfo, solutionMapping, solution):
        ...
        if actualMoveCostsI != relocateCost:
            ...
            print(
                "ERROR: Actual cost change per route not the same as solution cost change: %i + %i != %i"
                % (actualMoveCostsI, 0, relocateCost)
            )
            print(solution[routeI]["Trips"])
    ...
    def _testRelocateMoveEndRoute(self, moveInfo, solutionMapping, solution):
        ...
        if moveType != "relocateBeforeDummy":
            errorsFound = True
            print(
                "ERROR: Incorrect move test (%s) for this kind of move (relocateBeforeDummy)."
                % moveType
            )
    ...
    def _testCrossMove(self, moveInfo, solutionMapping, solution):
        ...
        if moveType.find("cross") == -1:
            errorsFound = True
            print(
                "ERROR: Incorrect move test (%s) for this kind of move (cross)."
                % moveType
            )
        ...
        if arcI != arc1 and self._inv[arcI] != arc1:
            errorsFound = True
            print(
                "ERROR: Arc to cross not the same as the one currently in the route position %i: actual %i vs %i"
                % (routeI, arc1, arcI)
            )
            print(solution[routeI]["Trips"])
        ...
        if preArc1 != preArcI:
            errorsFound = True
            print(
                "ERROR: Arc now before cross arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeI, preArc1, preArcI)
            )
            print(solution[routeI]["Trips"])
        ...
        if preArc2 != preArcJ:
            ...
            print(
                "ERROR: Arc now before insert at arc not the one used for calculating move cost in route %i: actual %i vs %i"
                % (routeJ, preArc2, preArcJ)
            )
            print(solution[routeJ]["Trips"])

        if moveType.find("_1crossPostIF") != -1:
            actualMoveCostsI = (
                self._if_cost[preArc1][arc2] - self._if_cost[preArc1][arc1]
            )
        else:
        ...
        if actualMoveCostsI != moveCostsI:
            errorsFound = True
            print(
                "ERROR: Calculated cross move cost not the same as given for route %i: actual %i vs %i"
                % (routeI, actualMoveCostsI, moveCostsI)
            )
            print(solution[routeI]["Trips"])
        ...
        if moveType.find("_2crossPostIF") != -1:
            actuaMoveCostsJ = (
                self._if_cost[preArc2][arc1] - self._if_cost[preArc2][arc2]
            )
        else:
    ...
    def _testCrossAtDummyMove(self, moveInfo, solutionMapping, solution):
        ...
        if moveType != "crossAtDummy":
            errorsFound = True
            print(
                "ERROR: Incorrect move test (%s) for this kind of move (crossAtDummy)."
                % moveType
            )
        ...
        if actualMoveCostsI != moveCostsI:
            errorsFound = True
            print(
                "ERROR: Calculated dummy cross move cost not the same as given for route %i: actual %i vs %i"
                % (routeI, actualMoveCostsI, moveCostsI)
            )
            print(solution[routeI]["Route"])
...
def updateMCARPcumulitiveSolution(info, solution, routeI):
    (
        solution[routeI]["CumLoad"],
        solution[routeI]["CumServe"],
    ) = genMCARPTIFcumulativeRoute(info, solution[routeI]["Trips"])
...
def addMCARPcumulativeSolution(info, solution):
    ...
    for i in range(solution["nVehicles"]):
        ...
        (solution[i]["CumLoad"], solution[i]["CumServe"]) = genMCARPTIFcumulativeRoute(
            info, route
        )
...
def printMove(
    ...
    if moveInfo[2].find("relocate") != -1:
        ...
        print(
            "Z: %i \t Savings: %i \t Relocate arc %i (R %i, T %i, P %i) before arc %i (R %i, T %i, P %i) \t %s \t %s"
            % (
                tCost,
                relocateCost,
                arcI,
    ...
    if moveInfo[2].find("exchange") != -1:
        ...
        print(
            "Z: %i \t Savings: %i \t Exchange arc %i (R %i, T %i, P %i) with arc %i (R %i, T %i, P %i) \t %s \t %s"
            % (
                tCost,
                relocateCost,
                arcI,
    ...
    if moveInfo[2] == "doubleRelocate":
        (
            relocateCost,
            (arcIa, preArcI, postArcI, arcJ, preArcJ, arcIb),
            moveType,
            (moveCostsI, moveCostsJ),
        ) = moveInfo
        print(
            "Z: %i \t Savings: %i \t Double relocate arcs %i and %i (R %i, T %i, P %i) before arc %i (R %i, T %i, P %i) \t %s"
            % (
                tCost,
                relocateCost,
                arcIa,
    ...
    if moveInfo[2].find("flip") != -1:
        ...
        print(
            "Z: %i \t Savings: %i \t Flip arc %i (R %i, T %i, P %i) \t %s \t %s"
            % (tCost, relocateCost, arcI, routeI, tripI, posI, moveType, _searchPhase)
        )
    ...
    if moveInfo[2].find("cross") != -1:
        if routeI == routeJ:
            ...
            print(
                "Z: %i \t Savings: %i \t Trip cross arc %i (R %i, T %i, P %i) to end with arc %i (R %i, T %i, P %i) to end \t %s %s \t %s"
                % (
                    tCost,
                    relocateCost,
                    arcI,
            ...
            print(
                "Z: %i \t Savings: %i \t Route cross arc %i (R %i, T %i, P %i) to end with arc %i (R %i, T %i, P %i) to end \t %s %s \t %s"
                % (
                    tCost,
                    relocateCost,
                    arcI,
...
class CalcMoveCosts(object):
    ...
    def __init__(self, info, nnList, cModules=True, autoInvCosts=False):
        self._info = info
        self._MoveCosts = calcMoveCost.CalcMoveCosts(
            info, cModules=cModules, nnList=nnList
        )
        self._MoveCosts_MCARPTIF = calcMoveCost.CalcMoveCostsMCARPTIFspecial(
            info, cModules=cModules, nnList=nnList
        )
    ...
    def setInputValues(self, threshold=None, nnFrac=None):
        if threshold:
            self.thresholdUse = threshold
        else:
            self.thresholdUse = self.threshold
        if nnFrac:
            self.nnFracUse = nnFrac
        else:
            self.nnFracUse = self.nnFrac
    ...
    def setSolution(self, solution):
        ...
        (
            ...
            self._dummyIFArcPositions,
        ) = self._genMCARPTIFgiantRouteMapping(solution)
        (
    ...
    def _genMCARPTIFgiantRouteMapping(self, solution):
        ...
        for i in range(solution["nVehicles"]):
            ...
            for j in range(nSubtrips):
                ...
                nArcs = len(subtrip)
                for n, arc in enumerate(
                    subtrip
                ):  # Each route consists of a start and end depot visit, with only one needed in the giant route.
                    ...
                    if j == 0 and n == 1:
                        ...
                        if arc in self._edgesS:
                            beginRouteArcsF.add(self._inv[arc])
                        normalPosition = False
                    if j > 0 and n == 1:
                        beginTripArcs.add(arc)
                        beginTripArcsF.add(arc)
                        if arc in self._edgesS:
                            beginTripArcsF.add(self._inv[arc])
                        normalPosition = False
                    if j == nSubtrips - 1 and n >= len(subtrip) - 3:
                        ...
                        if n == len(subtrip) - 3:
                            ...
                            if arc in self._edgesS:
                                endRouteArcsF.add(self._inv[arc])
                    if j < nSubtrips - 1 and n >= len(subtrip) - 2:
                        ...
                        if n == len(subtrip) - 2:
                            ...
                            if arc in self._edgesS:
                                endTripArcsF.add(self._inv[arc])
                    ...
                    if j == nSubtrips - 1 and n == nArcs - 2:
                        ...
                        if normalPosition == True:
                            ...
                            normalArcsF.add(arc)
                            if arc in self._edgesS:
                                normalArcsF.add(self._inv[arc])

        singleRouteArcs = beginRouteArcs.intersection(endRouteArcs)
        singleTripArcs = (beginRouteArcs.union(beginTripArcs)).intersection(
            endRouteArcs.union(endTripArcs)
        )
        ...
        verySpecialArcs = singleRouteArcsF.union(singleTripArcsF).union(
            singleRouteArcsF
        )
        ...
        return (
            ...
            normalArcs,
            normalArcsF,
            dummyDepotArcPositions,
            dummyIFArcPositions,
        )

    def calcRelocatePreIFMoves(
        self, relocateCandidates=None, insertCandidates=None, dummyArcPositions=None
    ):
        ...
        if "relocatePreIF" in self.movesToUse:
            ...
            if relocateCandidates == None:
                relocateCandidates = self._relocateCandidates.difference(
                    self.verySpecialArcs
                )

            if insertCandidates == None:
                insertCandidates = (
                    self._endRouteArcs.union(self._endTripArcs)
                    .union(self.singleTripArcs)
                    .union(self.singleRouteArcs)
                )

            if self.nonAdjacentRestrictionInsert:
                insertCandidates = insertCandidates.intersection(
                    self.nonAdjacentInsertArcs
                )

            savings += self._MoveCosts_MCARPTIF.relocateMovesPreIF(
                relocateCandidates,
                insertCandidates,
                threshold=self.thresholdUse,
                nNearest=self.nnFracUse,
            )

        return savings

    def calcRelocatePostIFMoves(
        self, relocateCandidates=None, insertCandidates=None, dummyArcPositions=None
    ):
        ...
        if "relocatePostIF" in self.movesToUse:
            ...
            if self.nonAdjacentRestrictionInsert:
                insertCandidates = insertCandidates.intersection(
                    self.nonAdjacentInsertArcs.difference(self.verySpecialArcs)
                )

            savings += self._MoveCosts_MCARPTIF.relocateMovesPostIF(
                relocateCandidates,
                insertCandidates,
                threshold=self.thresholdUse,
                nNearest=self.nnFracUse,
    ...
    def calcRelocateMoves(
        ...
        if "relocate" in self.movesToUse:
            ...
            if insertCandidates == None:
                insertCandidates = (
                    self._normalArcs.union(self._endRouteArcs)
                    .union(self._endTripArcs)
                    .union(self._beginRouteArcs)
                    .difference(self.verySpecialArcs)
                )
            ...
            savings += self._MoveCosts_MCARPTIF.relocateMovesMCARPTIF(
                relocateCandidates,
                insertCandidates,
                threshold=self.thresholdUse,
                nNearest=self.nnFracUse,
    ...
    def calcDoubleRelocateMoves(
        ...
        if "doubleRelocate" in self.movesToUse:
            ...
            if relocateCandidates == None:
                ...
                # else:
                relocateCandidates = self._solutionArcs.difference(
                    self._endTripArcs
                ).difference(self._endRouteArcs)
            ...
            savings += self._MoveCosts.doubleRelocateMoves(
                relocateCandidates,
                insertCandidates,
                threshold=self.thresholdUse,
                nNearest=self.nnFracUse,
    ...
    def calcExchangeMoves(self, exchangeCandidates1=None, exchangeCandidates2=None):
        ...
        if "exchange" in self.movesToUse:
            ...
            if exchangeCandidates1 == None:
                exchangeCandidates1 = self._relocateCandidates.difference(
                    self.verySpecialArcs
                )

            if exchangeCandidates2 == None:
                exchangeCandidates2 = self._relocateCandidates.difference(
                    self.verySpecialArcs
                )

            if self.nonAdjacentRestrictionExchange:
                exchangeCandidates1 = exchangeCandidates1.intersection(
                    self.nonAdjacentExchangeArcs
                )
                exchangeCandidates2 = exchangeCandidates2.intersection(
                    self.nonAdjacentExchangeArcs
                )

            savings += self._MoveCosts_MCARPTIF.exchangeMovesMCARPTIF(
                exchangeCandidates1,
                exchangeCandidates2,
                threshold=self.thresholdUse,
                nNearest=self.nnFracUse,
    ...
    def calcCrossMoves(self, crossCandidates1=None, crossCandidates2=None):
        ...
        if "cross" in self.movesToUse:
            ...
            if crossCandidates1 == None:
                crossCandidates1 = self._solutionArcs.difference(
                    self.verySpecialArcs
                ).difference(self._beginTripArcs)

            if crossCandidates2 == None:
                crossCandidates2 = self._solutionArcs.difference(
                    self.verySpecialArcs
                ).difference(self._beginTripArcs)

            if self.nonAdjacentRestrictionCross:
                crossCandidates1 = crossCandidates1.intersection(
                    self.nonAdjacentInsertArcs
                )
                crossCandidates2 = crossCandidates2.intersection(
                    self.nonAdjacentInsertArcs
                )

            savings += self._MoveCosts_MCARPTIF.crossMovesMCARPTIF(
                crossCandidates1,
                crossCandidates2,
                threshold=self.thresholdUse,
                nNearest=self.nnFracUse,
    ...
    def calcFlipMoves(self, flipCandidates=None):
        ...
        if "flip" in self.movesToUse:
            ...
            if flipCandidates == None:
                flipCandidates = self._solutionArcs.intersection(
                    self._edgesS
                ).difference(self.verySpecialArcs)

            savings += self._MoveCosts_MCARPTIF.flipMovesMCARPTIF(
                flipCandidates, threshold=self.thresholdUse
            )
    ...
    def calcSingleRouteCrossCosts(self, routeSavings, solution):
        ...
        for route in routeSavings:
            ...
            savings += self._MoveCosts.crossMoves(
                solutionArcs,
                solutionArcs,
                threshold=-thresholdUse,
                nNearest=self.nnFracUse,
            )
        return savings

    def calcSingleRouteOutCosts(
        self,
        routeSavings,
        solution,
        relocateCandidates=None,
        solutionArcs=None,
        dummyArcPositions=None,
    ):
        """
        Calculate the cost of neighbourhood moves to current solution.
        """
        ...
        for route in routeSavings:
            ...
            routeCandidates += [
                self._inv[arc] for arc in routeArcs if arc in self._edgesS
            ]
            ...
            savings += self._MoveCosts.relocateMoves(
                relocateCandidates,
                relocateToCandidates,
                threshold=thresholdUse,
                nNearest=self.nnFracUse,
            )
            savings += self._MoveCosts.relocateEndRouteMoves(
                relocateCandidates,
                dummyArcPositionsTemp,
                threshold=thresholdUse,
                nNearest=self.nnFracUse,
            )
    ...
    def _setNonAdjacentArcs(self):
        ...
        if (
            self.nonAdjacentRestrictionCross
            or self.nonAdjacentRestrictionExchange
            or self.nonAdjacentRestrictionInsert
        ):
...
class MovesFeasibleMCARPTIF(object):
    ...
    def _relocateFeasibility(
        self, moveInfo, solutionMapping, solution, loadCheck=False
    ):
        """
        ...
        if moveType.find("relocate") == -1:
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (relocate)."
                % moveType
            )
        ...
        excessI = (
            solution[routeI]["TripLoads"][tripI] + deltaChangeRouteI - self._capacity
        )
        excessJ = (
            solution[routeJ]["TripLoads"][tripJ] + deltaChangeRouteJ - self._capacity
        )

        moveFeasible = excessI <= 0 and excessJ <= 0
        if not moveFeasible:
            moveToMake = "nomoveLoad"
        else:
            moveToMake = "feasible"
        ...
        if moveFeasible or loadCheck:
            ...
            if routeI != routeJ:
                ...
                excessCostI = (
                    solution[routeI]["Cost"]
                    + delateServiceRouteI
                    + netCostRouteI
                    - self._maxTrip
                )
                excessCostJ = (
                    solution[routeJ]["Cost"]
                    + delateServiceRouteJ
                    + netCostRouteJ
                    - self._maxTrip
                )
            else:
                excessCostI = excessCostJ = (
                    solution[routeI]["Cost"] + moveCost - self._maxTrip
                )
    ...
    def _doubleRelocateFeasibility(self, moveInfo, solutionMapping, solution):
        ...
        (
            moveCost,
            (arcIa, preArcI, postArcI, arcJ, preArcJ, postArcJ),
            moveType,
            routeCosts,
        ) = moveInfo
        (netCostRouteI, netCostRouteJ) = routeCosts
        ...
        if moveType != "doubleRelocate":
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (doubleRelocate)."
                % moveType
            )
        ...
        if moveFeasible:
            ...
            if routeI != routeJ:
                delateServiceRouteI = -(
                    self._servCost[arcIa]
                    + self._d[arcIa][arcIb]
                    + self._servCost[arcIb]
                )
    ...
    def _exchangeFeasibility(
        ...
        if moveType.find("exchange") == -1:
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (exchange)."
                % moveType
            )
        (netCostRouteI, netCostRouteJ) = routeCosts
    ...
    def _flipFeasibility(self, moveInfo, solutionMapping, solution):
        ...
        if moveType.find("flip") == -1:
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (flip)."
                % moveType
            )
        (routeI, tripI, posI) = solutionMapping[arcI]
    ...
    def _testCrossMove(self, moveInfo, solutionMapping, solution):
        ...
        if tripI == len(solutionTemp[routeI]["Trips"]) - 1:
            iEnd = -2
            endAddI = [
                solutionTemp[routeI]["Trips"][tripI][-2],
                solutionTemp[routeI]["Trips"][tripI][-1],
            ]
        else:
            iEnd = -1
        ...
        if tripJ == len(solutionTemp[routeJ]["Trips"]) - 1:
            jEnd = -2
            endAddJ = [
                solutionTemp[routeJ]["Trips"][tripJ][-2],
                solutionTemp[routeJ]["Trips"][tripJ][-1],
            ]
        else:
            jEnd = -1
        ...
        if (
            solutionTemp[routeI]["Trips"][tripI][iEnd]
            == solutionTemp[routeJ]["Trips"][tripJ][jEnd]
        ):
            tripMove = "TripMove"
    ...
    def _crossFeasibility(self, moveInfo, solutionMapping, solution):
        ...
        if moveType.find("cross") == 1:
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (cross)."
                % moveType
            )
        (routeI, tripI, posI) = solutionMapping[arcI]
        (routeJ, tripJ, posJ) = solutionMapping[arcJ]
        ...
        if routeI == routeJ and tripI == tripJ:
            ...
            solution[routeI]["Trips"][tripI][tripIend]
            == solution[routeJ]["Trips"][tripJ][tripJend]
            and tripIend == tripJend
        ):
            moveFeasible = True
            tripCross = True
        elif routeI == routeJ:
        ...
        if moveFeasible:
            ...
            excessI = (
                solution[routeI]["TripLoads"][tripI]
                + deltaChangeRouteI
                - self._capacity
            )
            excessJ = (
                solution[routeJ]["TripLoads"][tripJ]
                + deltaChangeRouteJ
                - self._capacity
            )
        ...
        if moveFeasible:
            ...
            if tripCross:
                ...
                secICostEnd = (
                    solution[routeI]["CumServe"][tripI][-1]
                    - secICostStart
                    - tripI_minOld
                )
                secJCostStart = solution[routeJ]["CumServe"][tripJ][posJ - 1]
                secJCostEnd = (
                    solution[routeJ]["CumServe"][tripJ][-1]
                    - secJCostStart
                    - tripJ_minOld
                )
            else:
                secICostStart = (
                    sum([s[-1] for s in solution[routeI]["CumServe"][:tripI]])
                    + solution[routeI]["CumServe"][tripI][posI - 1]
                )
                secICostEnd = solution[routeI]["Cost"] - secICostStart - tripI_minOld
                secJCostStart = (
                    sum([s[-1] for s in solution[routeJ]["CumServe"][:tripJ]])
                    + solution[routeJ]["CumServe"][tripJ][posJ - 1]
                )
            ...
            #
            excessCostI = (
                solution[routeI]["Cost"] + deltaCostChangeRouteI - self._maxTrip
            )
            excessCostJ = (
                solution[routeJ]["Cost"] + deltaCostChangeRouteJ - self._maxTrip
            )
            ...
            if not moveFeasible:
                if tripCross:
                    moveToMake = "nomoveTripCost"
                else:
                    moveToMake = "nomoveRouteCost"
            else:
                if tripCross:
                    moveToMake = "feasibleTrip"
                else:
                    moveToMake = "feasibleRoute"
    ...
    def _doubleCrossFeasibility(self, moveInfo, solutionMapping, solution):
        ...
        if moveType != "doubleCross":
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (doubleCross)."
                % moveType
            )

        (
            crossCost1,
            (arcI1, preArcI1, postArcI1, arcJ1, preArcJ1, postArcJ1),
            moveType,
            (routeIcostChange, routeJcostChange),
        ) = moveInfo1
        if moveType.find("cross") == -1:
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (cross)."
        ...
        (
            crossCost2,
            (arcI2, preArcI2, postArcI2, arcJ2, preArcJ2, postArcJ2),
            moveType,
            (routeIcostChange, routeJcostChange),
        ) = moveInfo2
        if moveType.find("cross") == -1:
        ...
        if routeI1 != routeJ1 or tripI1 != tripJ1:
            return ("nomove", 0, 0)
        if routeI2 != routeJ2 or tripI2 != tripJ2:
            return ("nomove", 0, 0)
        if routeI1 != routeI2 or tripI1 != tripI2:
            return ("nomove", 0, 0)
    ...
    def doubleMoveFeasibility(self, moveInfo, solutionMapping, solution):
        ...
        if moveType != "comboMove":
            raise NameError(
                "Incorrect feasibility check (%s) for this kind of move (comboMove)."
                % moveType
            )
        # solutionLoads = {i : {'Load' : solution[i]['Load']} for i in range(solution['nVehicles'])}
        solutionLoads = deepcopy(solution)

        (routeI1, tripI1, routeJ1, tripJ1) = self._returnRouteInfo(
            moveInfo1, solutionMapping, solution
        )

        (feasible1, loadI, loadJ) = self.checkMoveFeasibility(
            moveInfo1,
            solutionMapping,
            solutionLoads,
            saveMoveInfo=False,
        ...
        if feasible1.find("feasible") == -1:
            return "nomove"

        solutionLoads[routeI1]["TripLoads"][tripI1] += loadI
        solutionLoads[routeJ1]["TripLoads"][tripJ1] += loadJ

        (routeI2, tripI2, routeJ2, tripJ2) = self._returnRouteInfo(
            moveInfo2, solutionMapping, solution
        )

        (feasible2, loadI, loadJ) = self.checkMoveFeasibility(
            moveInfo2,
            solutionMapping,
            solutionLoads,
            saveMoveInfo=False,
        ...
        feasible1 = (
            solutionLoads[routeI1]["TripLoads"][tripI1] <= self._capacity
            and solutionLoads[routeJ1]["TripLoads"][tripJ1] <= self._capacity
        )
        feasible2 = (
            solutionLoads[routeI2]["TripLoads"][tripI2] <= self._capacity
            and solutionLoads[routeJ2]["TripLoads"][tripJ2] <= self._capacity
        )
    ...
    def checkMoveFeasibility(
        self, moveInfo, solutionMapping, solution, saveMoveInfo=True, loadCheck=False
    ):
        """
        ...
        self._solution = solution
        if moveType.find("relocate") != -1:
            (feasible, loadI, loadJ) = self._relocateFeasibility(
                moveInfo, solutionMapping, solution, loadCheck
            )
        if moveType.find("exchange") != -1:
            (feasible, loadI, loadJ) = self._exchangeFeasibility(
                moveInfo, solutionMapping, solution, loadCheck
            )
        if moveType.find("cross") != -1:
            (feasible, loadI, loadJ, solution) = self._crossFeasibility(
                moveInfo, solutionMapping, solution
            )
        if moveType.find("flip") != -1:
            (feasible, loadI, loadJ, solution) = self._flipFeasibility(
                moveInfo, solutionMapping, solution
            )
        if moveType == "doubleCross":
            (feasible, loadI, loadJ) = self._doubleCrossFeasibility(
                moveInfo, solutionMapping, solution
            )

        if moveType == "doubleRelocate":
            (feasible, loadI, loadJ) = self._doubleRelocateFeasibility(
                moveInfo, solutionMapping, solution
            )
        if moveType == "comboMove":
            (feasible, loadI, loadJ) = self._doubleMoveFeasibility(
                moveInfo, solutionMapping, solution
            )

        if moveType != "cross" and saveMoveInfo:
            if feasible == "nomoveLoad":
                self.infeasibleLoadMoves.append((moveInfo[0], loadI, loadJ, moveInfo))
            if feasible == "nomoveCost":
                self.infeasibleCostMoves.append((moveInfo[0], loadI, loadJ, moveInfo))
            if moveType.find("relocate") != -1 and moveInfo[0] >= self.threshold:
                self.exceedThresholdRemoveInsert.append(moveInfo)

        if (
            moveType.find("cross") != -1
            and feasible == "nomoveSameRouteSameTrip"
            and saveMoveInfo
        ):
...
class MakeMovesMCARPTIF(object):
    ...
    def _genMCARPTIFsolutionMapping(self):
        ...
        solutionMapping = (
            {}
        )  # Mapping of an arc to its position (route and route position) in the solution.
        for i in range(solution["nVehicles"]):
            nSubtrips = len(solution[i]["Trips"])
            for j in range(nSubtrips):
                subtrip = solution[i]["Trips"][j]
                for n, arc in enumerate(
                    ...
                    solutionMapping[arc] = (
                        ...
                        j,
                        n,
                    )  # Since positioning starts at the first arc after the depot, j is increased by one.
                    if arc in self._edgesS:
                        solutionMapping[self._inv[arc]] = (
                            i,
                            j,
                            n,
                        )  # Position of inverse arc is also captured, thus allowing for arc inversion (should come more into play with windy edges.
    ...
    def _updateCrossSolution(self, routeI):
        ...
        for i in range(nTrips):
            self.solution[routeI]["TripLoads"] += [
                self.solution[routeI]["CumLoad"][i][-1]
            ]
    ...
    def _updateArcPositions(self, routeI, tripI):
        ...
        for j, arc in enumerate(
            self.solution[routeI]["Trips"][tripI][1:-1]
        ):  # Note that j will start at zero, not 1
            self.solutionMapping[arc] = (
                routeI,
                tripI,
                j + 1,
            )  # Since positioning starts at the first arc after the depot, j is increased by one.
    ...
    def _relocateMove(self):
        ...
        """
        if self._testAll:
            self._testMoves._testRelocateMove(
                self._moveInfo, self.solutionMapping, self.solution
            )
        ...
        if routeI != routeJ or tripI != tripJ:
            self._updateArcPositions(routeJ, tripJ)
    ...
    def _relocateMoveIF(self):
        ...
        """
        if self._testAll:
            self._testMoves._testRelocateIFMove(
                self._moveInfo, self.solutionMapping, self.solution
            )
    ...
    def _doubleRelocateMove(self):
        ...
        """
        if self._testAll:
            self._testMoves._testDoubleRelocateMove(
                self._moveInfo, self.solutionMapping, self.solution
            )
    ...
    def _exchangeMove(self):
        ...
        """
        if self._testAll:
            self._testMoves._testExchangeMove(
                self._moveInfo, self.solutionMapping, self.solution
            )
        ...
        if arcJ in self._edgesS:
            self.solutionMapping[self._inv[arcJ]] = (routeI, tripI, arcPos1)

        self.solutionMapping[arcI] = (routeJ, tripJ, arcPos2)
        if arcI in self._edgesS:
            self.solutionMapping[self._inv[arcI]] = (routeJ, tripJ, arcPos2)

        self._printMove(
            routeI, tripI, arcPos1, routeJ, tripJ, arcPos2, self.solution["TotalCost"]
        )
        extraInfo = ""
        return extraInfo
    ...
    def _flipMove(self):
        ...
        self._printMove(
            routeI, tripI, arcPos1, None, None, None, self.solution["TotalCost"]
        )
        extraInfo = ""
        return extraInfo
    ...
    def _crossMove(self):
        ...
        """
        if self._testAll:
            self._testMoves._testCrossMove(
                self._moveInfo, self.solutionMapping, self.solution
            )
        ...
        if tripI == len(self.solution[routeI]["Trips"]) - 1:
            iEnd = -2
            endAddI = [
                self.solution[routeI]["Trips"][tripI][-2],
                self.solution[routeI]["Trips"][tripI][-1],
            ]
        else:
            iEnd = -1
        ...
        if tripJ == len(self.solution[routeJ]["Trips"]) - 1:
            jEnd = -2
            endAddJ = [
                self.solution[routeJ]["Trips"][tripJ][-2],
                self.solution[routeJ]["Trips"][tripJ][-1],
            ]
        else:
            jEnd = -1
        ...
        if (
            self.solution[routeI]["Trips"][tripI][iEnd]
            == self.solution[routeJ]["Trips"][tripJ][jEnd]
        ):
            tripMove = "TripMove"
        ...
        if tripI < len(self.solution[routeI]["Trips"]) - 1:
            ...
            if bestPostIFI != postIF:
                prevLinkCost = (
                    self._d[postArc][postIF]
                    + self._d[postIF][postPostArc]
                    + self._dumpCost
                )
    ...
    def _doubleCrossMove(self):
        ...
        if self._testAll:
            self._testMoves._testCrossMove(
                moveInfo1, self.solutionMapping, self.solution
            )
        if self._testAll:
            self._testMoves._testCrossMove(
                moveInfo2, self.solutionMapping, self.solution
            )
        ...
        if routeI != routeJ or routeI2 != routeJ2 or routeI != routeI2:
            raise NameError(
                "Double cross move cannot be applied to different routes: %i and %i and %i and %i"
                % (routeI, routeJ, routeI2, routeJ2)
            )
        if tripI != tripJ or tripI2 != tripJ2 or tripI != tripI2:
            raise NameError(
                "Double cross move cannot be applied to different trips: %i and %i and %i and %i"
                % (tripI, tripJ, tripI2, tripJ2)
            )
    ...
    def makeMove(self, moveInfo, solution):
        ...
        if moveType == "doubleRelocate":
            self._doubleRelocateMove()
        if moveType == "relocateBeforeDummy":
            self._relocateBeginnigRouteMove()
        if moveType == "crossAtDummy":
            self._crossAtDummyMove()
        if moveType == "doubleCross":
            self._doubleCrossMove()
        if moveType == "comboMove":
            self._comboMove()
        self._moveInfo = []
        if printSolution:
...
class DoubleCrossMovesMCARPTIF(object):
    ...
    def pairFeasibleDoubleCrossMoves(self, solution, solutionMapping, doubleCross):
        ...
        routesSavings = self.findDoubleCrossMoveCosts(
            solution, solutionMapping, doubleCross
        )
        for (route, trip) in routesSavings:
            ...
            for i in range(nSavings - 1):
                ...
                for j in range(i, nSavings):
                    ...
                    if tCost >= self.threshold:
                        continue
                    moveInfo = (tCost, (moveInfo1, moveInfo2), "doubleCross")
                    feasible = self._checkFeasibility.checkMoveFeasibility(
                        moveInfo, solutionMapping, solution
                    )[0]
...
class InfeasibleMovesMCARPTIF(object):
    ...
    def infeasibleMoveCombos(
        self, solution, solutionMapping, infeasibleMoves, exceedThreshold
    ):

        CompoundTemp = CompoundMoveFunctions(self.info, False)
        infeasibleGrouping = self.findInfeasibleMoveCosts(
            solution, solutionMapping, infeasibleMoves
        )
        ...
        for (routeI, tripI) in infeasibleGrouping:
            if (routeI, tripI) not in outGrouping:
                continue
            for InfeasibleMoveInfo in infeasibleGrouping[(routeI, tripI)]:
                ...
                for outMoveInfo in outGrouping[(routeI, tripI)]:
                    if moveInfo[0] + outMoveInfo[0] >= self.threshold:
                        continue
                    compoundComboOutFeasible = (
                        CompoundTemp.checkCompoundMoveFeasibility(outMoveInfo)
                    )
                    if compoundComboOutFeasible:
                        savingsCombo.append(
                            (
                                moveInfo[0] + outMoveInfo[0],
...
class CompoundMoveFunctions:
    ...
    def _genMCARPTIFclassifications(self, solution):
        ...
        self.verySpecialArcs = singleRouteArcsF.union(singleTripArcsF).union(
            singleRouteArcsF
        )
    ...
    def checkCompoundMoveFeasibility(self, moveInfo):
        ...
        if moveType == "doubleRelocate":
            ...
            arcI2 = postArcJ
            if (
                arcI1 in self._locationChanged
                or arcI2 in self._locationChanged
                or arcJ in self._preLocationChanged
            ):
                feasible = False
        ...
        if moveType.find("relocate") != -1:
            if moveType.find("_PreIF") != -1:
                if (
                    arcJ in self._preLocationChanged
                    or postArcJ in self._preLocationChanged
                ):
                    feasible = False
    ...
    def checkCompoundDoubleMoveFeasibility(self, moveInfo):
        ...
        feasible = self.checkCompoundMoveFeasibility(
            moveInfo1
        ) and self.checkCompoundMoveFeasibility(moveInfo2)
        return feasible
    ...
    def addMoveInfluence(
        self, moveInfo, solution=None, solutionMapping=None, feasible=""
    ):
        ...
        self._locationChanged.update(
            self.addArc({preArcI, arcI, postArcI, preArcJ, arcJ, postArcJ})
        )
        self._preLocationChanged.update(
            self.addArc({preArcI, arcI, postArcI, preArcJ, arcJ, postArcJ})
        )
        ...
        if feasible.find("feasibleTrip") != -1:
            ...
            self._locationChanged.update(
                self.addArc({preArcI, postArcI, preArcJ, postArcJ})
            )
            self._preLocationChanged.update(
                self.addArc({preArcI, postArcI, preArcJ, postArcJ})
            )
    ...
    def returnRelocateArcs(self):
        relocateArcs = self._locationChanged.intersection(self._reqArcs).difference(
            self.verySpecialArcs
        )
        relocateArcsDiff = self._reqArcs.difference(relocateArcs).difference(
            self.verySpecialArcs
        )
        return (relocateArcs, relocateArcsDiff)

    def returnRelocateArcsNormal(self):
        relocateArcs = self._locationChanged.intersection(
            self.normalArcsF.union(self.beginRouteArcsF)
        )
        normalArcsDiff = (self.normalArcsF.union(self.beginRouteArcsF)).difference(
            relocateArcs
        )
        return (relocateArcs, normalArcsDiff)

    def returnInsertPositionsNormal(self):
        insertLocations = self._locationChanged.intersection(
            self.normalArcs.union(self.beginRouteArcs)
            .union(self.endRouteArcs)
            .union(self.endTripArcs)
        ).difference(self.verySpecialArcs)
        return insertLocations

    def returnInsertPositionsPreIF(self):
        insertLocations = self._locationChanged.intersection(
            self.endTripArcs.union(self.endRouteArcs)
            .union(self.singleTripArcs)
            .union(self.singleRouteArcs)
        )
        return insertLocations
    ...
    def returnSolutionArcs(self):
        solutionArcs = self._locationChanged.intersection(self.solutionArcs).difference(
            self.verySpecialArcs
        )
        return solutionArcs
    ...
    def returnCrossArcs(self):
        solutionArcs = (
            self._locationChanged.intersection(self.solutionArcs)
            .difference(self.verySpecialArcs)
            .difference(self.beginTripArcs)
        )
        return solutionArcs
    ...
    def reduceFleet(self, solution):
        ...
        if self._reduceAllTrips and solution["nVehicles"] > self._targetFleetSize:
            ...
            if reduced:
                ...
                s = (
                    "\n Saving: %i \t Type: %s \t Fleet reduced by: %i from %i to %i"
                    % outS
                )
...
class LS_MCARPTIF(object):
    ...
    def setNonAdjacentArcRestriction(self):
        ...
        self._MoveCosts.nonAdjacentRestrictionExchange = (
            self.nonAdjacentRestrictionExchange
        )
    ...
    def _candidateCalculations(self, solution):
        ...
        self._movesNotUsed = self._MovesCompound.returnNonCompoundMoves(
            self._movesNotUsed
        )
        ...
        savings += self._MoveCosts.calcRelocateMoves(
            relocateCandidates=relocateArcs, insertCandidates=None
        )
        savings += self._MoveCosts.calcRelocateMoves(
            relocateCandidates=relocateArcsDiff, insertCandidates=insertPositionsNormal
        )
        ...
        savings += self._MoveCosts.calcRelocatePreIFMoves(
            relocateCandidates=relocateArcs, insertCandidates=None
        )
        savings += self._MoveCosts.calcRelocatePreIFMoves(
            relocateCandidates=relocateArcsDiff, insertCandidates=insertPositionsPreIF
        )
        ...
        savings += self._MoveCosts.calcRelocatePostIFMoves(
            relocateCandidates=relocateArcs, insertCandidates=None
        )
        savings += self._MoveCosts.calcRelocatePostIFMoves(
            relocateCandidates=relocateArcsDiff, insertCandidates=insertPositionsPostIF
        )

        savings += self._MoveCosts.calcExchangeMoves(
            exchangeCandidates1=relocateArcs, exchangeCandidates2=None
        )
        savings += self._MoveCosts.calcExchangeMoves(
            exchangeCandidates1=None, exchangeCandidates2=relocateArcs
        )  # optional

        crossArcs = self._MovesCompound.returnCrossArcs()
        savings += self._MoveCosts.calcCrossMoves(
            crossCandidates1=crossArcs, crossCandidates2=None
        )
        savings += self._MoveCosts.calcCrossMoves(
            crossCandidates1=None, crossCandidates2=crossArcs
        )  # optional
    ...
    def _makeCompoundMoves(self, savings, solution):
        ...
        for iMove, moveInfo in enumerate(savings):
            compoundFeasible = self._MovesCompound.checkCompoundMoveFeasibility(
                moveInfo
            )
            if not compoundFeasible:
                continue

            solutionMapping = self._MovesMaker.solutionMapping
            (feasible, solution) = self._MovesFeasible.checkMoveFeasibility(
                moveInfo, solutionMapping, solution
            )[:2]
            ...
            self._MovesCompound.addMoveInfluence(
                moveInfo, solution, solutionMapping, feasible
            )
            (solution, extraInfo) = self._MovesMaker.makeMove(moveInfo, solution)

            if extraInfo == "TripMove":
                self._MovesCompound.addCrossMoveInfluence(
                    moveInfo, solution, solutionMapping
                )

            self._nMoves += 1

            if self._testAll:
                self._testMoves._testSolution(solution)
    ...
    def _makeDoubleCrossCompoundMoves(self, solution):
        ...
        self._MovesFeasible.doubleCrossMove = (
            self._MovesCompound.returnNonCompoundMoves(
                self._MovesFeasible.doubleCrossMove
            )
        )
        self._MovesFeasible.exceedThresholdDoubleCross = (
            self._MovesCompound.returnNonCompoundMoves(
                self._MovesFeasible.exceedThresholdDoubleCross
            )
        )
        ...
        savingsR = self._MovesDoubleCross.pairFeasibleDoubleCrossMoves(
            solution, self._MovesMaker.solutionMapping, doubleCrossMoves
        )
        feasibleMove = False
        ...
        for route in savingsR:
            for moveInfo in savingsR[route]:
                ...
                if moveInfo[0] >= self.threshold:
                    continue
                compoundFeasible = (
                    self._MovesCompound.checkCompoundDoubleMoveFeasibility(moveInfo)
                )
                if not compoundFeasible:
                    continue
    ...
    def _makeInfeasibleCompoundMoves(self, solution):
        ...
        self._MovesMaker.setSolution(solution)
        self._MovesFeasible.infeasibleMoves = (
            self._MovesCompound.returnNonCompoundInfeasibleMoves(
                self._MovesFeasible.infeasibleLoadMoves
            )
        )
        self._MovesFeasible.exceedThresholdRemoveInsert = (
            self._MovesCompound.returnNonCompoundMoves(
                self._MovesFeasible.exceedThresholdRemoveInsert
            )
        )
        ...
        infeasibleMoveCombos = self._MovesInfeasible.infeasibleMoveCombos(
            solution, self._MovesMaker.solutionMapping, infeasibleMoves, costlyMoves
        )
        feasibleMove = False
        ...
        for comboMove in infeasibleMoveCombos:
            # print(comboMove)
            compoundFeasible = self._MovesCompound.checkCompoundDoubleMoveFeasibility(
                comboMove
            )
            # print(compoundFeasible)
            if not compoundFeasible:
                continue
            feasible = self._MovesFeasible.doubleMoveFeasibility(
                comboMove, self._MovesMaker.solutionMapping, solution
            )
    ...
    def compoundLocalSearch(self, solution):
        ...
        while True:
            ...
            if not self.comboMovesVN or (
                not True in feasibleMoves and self.comboMovesVN
            ):
                if self.doubleCross:
                    (feasibleMove, solution) = self._makeDoubleCrossCompoundMoves(
                        solution
                    )
                    feasibleMoves.append(feasibleMove)
                if self.comboFeasibleMoves:
                    (feasibleMove, solution) = self._makeInfeasibleCompoundMoves(
                        solution
                    )
                    feasibleMoves.append(feasibleMove)

            if not self.newComboMovesVN or (
                not True in feasibleMoves and self.newComboMovesVN
            ):
                if self.newDoubleCross:
                    (feasibleMove, solution) = self._makeNewDoubleCrossCompoundMoves(
                        solution
                    )
                    feasibleMoves.append(feasibleMove)
                if self.newComboFeasibleMoves:
                    (feasibleMove, solution) = self._makeNewInfeasibleCompoundMoves(
                        solution
                    )
                    feasibleMoves.append(feasibleMove)
            ...
            if not True in feasibleMoves:
                ...
                if self._printEachMove:
                    print(
                        "I: %i \t Saving: %i \t # Moves %i \t Moves made %i"
                        % (
                            self._nCompoundMoves,
                            self._solutionChange,
                            self._nMoves,
                            nMovesMade,
                        )
                    )
    ...
    def localSearch(
        self, solutionStart, nnFrac=1, candidateMoves=True, compoundMoves=True
    ):
        t = clock()
    ...
    def returnStats(self, pSet, initial, moveStrategy="ScanAllMoves"):
        if self.candidateMoves:
            canList = "TRUE"
        else:
            canList = "FALSE"
        if self.compoundMoves:
            compoundMove = "TRUE"
        else:
            compoundMove = "FALSE"
    ...
    def returnStatsFormat(self, pSet, initial, nSol, initTime):
        ...
        if self.comboFeasibleMoves and not self.comboMovesVN:
            moveStrategy = "FALSE"
        elif self.comboFeasibleMoves and self.comboMovesVN:
            moveStrategy = "TRUE"
        else:
            moveStrategy = "FALSE"
        ...
        outputLineStr = (
            "%s,%s,%s,%.2f,%s,%s,%i,%i,%i,%i,%.4f,%i,%i,%s,%s,%i,%.8f\n"
            % outputLineInfo
        )
        return outputLineStr

    def locaSearchImbedded(
        self, solutionStart
    ):  # , nnFrac = 1, candidateMoves = True, compoundMoves = True):
        t = clock()
        solution = deepcopy(solutionStart)
        self._initialCost = solution["TotalCost"]
        self._previousCost = solution["TotalCost"]
    ...
    def searchSummary(
        ...
        if self._printSummary:
            ...
            print(
                "# Iterations:       %i \t (per iteration: %.4f)"
                % (
                    self._nCompoundMoves,
                    self.executionTime / max([1, self._nCompoundMoves]),
                )
            )
            print(
                "# Moves:            %i \t (per move:      %.4f)"
                % (self._nMoves, self.executionTime / max([1, self._nMoves]))
            )
            print("================================================================\n")
...
class TabuFunctions:
    ...
    def setTabuInfo(self, nMoves=None, nCompoundMoves=None):
        if nMoves != None:
            self._nMoves = nMoves
        if nCompoundMoves != None:
            self._nCompoundMoves = nCompoundMoves

    def setTabuSettings(self, tabuTenure=None, compoundMoves=None):
        if tabuTenure:
            self._tabuTenure = tabuTenure
        if compoundMoves:
            self.compoundTabuList = compoundMoves
    ...
    def checkMoveNotTabu(self, moveInfo):
        ...
        if (
            moveType == "exchange"
            or moveType == "cross"
            or moveType == "relocate"
            or moveType == "doubleRelocate"
        ):
    ...
    def _updateTabuListMove(self, arc):
        ...
        if arc in self._edgesS:
            self._tabuListMove[self._inv[arc]] = self._nMoves + self.tabuTenure

    def _updateTabuListCompoundMove(self, arc):
        self._tabuListCompound[arc] = self._nCompoundMoves + self.tabuTenure
        if arc in self._edgesS:
            self._tabuListCompound[self._inv[arc]] = (
                self._nCompoundMoves + self.tabuTenure
            )
    ...
    def updateTabuList(self, moveInfo):
        ...
        if moveType == "doubleRelocate":
            ...
            self._updateTabuListCompoundMove(arcI)
        if (
            moveType == "exchange" or moveType == "cross" or moveType == "relocate"
        ):  # or moveType == 'relocate':
...
class TabuSearch_MCARPTIF(object):
    ...
    def setOutputString(
        self, problemSet, initial, experimentName, experimentNumber, outputFile
    ):
    ...
    def _makeTabuFeasibleCompoundMoves(self, savings, solution):
        ...
        tabuMoves = 0
        for iMove, moveInfo in enumerate(savings):
            compoundFeasible = self._LSfun._MovesCompound.checkCompoundMoveFeasibility(
                moveInfo
            )
            if not compoundFeasible:
                continue
            ...
            if feasible:
                ...
                (feasible, solution) = self._LSfun._MovesFeasible.checkMoveFeasibility(
                    moveInfo, solutionMapping, solution
                )[:2]
                if feasible.find("feasible") != -1:
                    feasible = True
            ...
            if extraInfo == "TripMove":
                self._LSfun._MovesCompound.addCrossMoveInfluence(
                    moveInfo, solution, solutionMapping
                )
            ...
            tabuMoves += 1
            if self._testAll:
                self._LSfun._testMoves._testSolution(solution)
    ...
    def _makeTabuMoves(self, savings, solution):
        ...
        (feasibleMove, solution) = self._makeTabuFeasibleCompoundMoves(
            savings, solution
        )
        feasibleMoves.append(feasibleMove)
    ...
    def moveSearch(self, solution, tLimit, timeStart, moveLimit):
        ...
        i = 0
        while (
            (self.totalTime < tLimit)
            and (i < moveLimit)
            and (self.nMovesSinceInc < self._nMovesNoImprovement)
        ):
            i += 1
            ...
            self.nMovesSinceInc = (
                self._TSfun._nCompoundMoves - self._TSfun._incK_nCM[-1]
            )
            ...
            if asspiration:
                (moveMade, solution, reducedFleet) = self._makeBestMoves(
                    savings, solution
                )
            else:
                moveMade = False
            ...
            if not moveMade:
                ...
                if p:
                    if not self.suppressOutput:
                        print(
                            "I: %i \t Saving: %i \t # Moves %i \t Inc: %i \t nMoves Inc: %i/%i \t Time: %.4f \t {NN, tau} = {%.2f,%i} "
                            % (
                                self._TSfun._nCompoundMoves,
                                self._solutionChange,
                                self._TSfun._nMoves,
    ...
    def compoundTabuSearch(self, solution, tLimit, moveLimit):
        ...
        while True:
            ...
            (solution, reducedFleet) = self.moveSearch(
                solution, tLimit, timeStart, moveLimit
            )
            if not reducedFleet:
                break
        ...
        if not self.suppressOutput:
            print(
                "\nInitial and incumbent cost: %i \t %i"
                % (self._TSfun._incK_z, self._initialCost)
            )
            print(
                "Initial and incumbent fleet size: %i \t %i\n"
    ...
    def improveSolution(self, solution):
        #
        self._initiateTabuSearch()
        solution = self.compoundTabuSearch(
            solution, tLimit=1e300000, moveLimit=1e300000
        )
        return deepcopy(self._TSfun._incKsol)
