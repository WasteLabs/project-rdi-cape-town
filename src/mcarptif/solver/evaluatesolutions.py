...
def _compare_solutions_on_criteria(
    candidate_solution, incumbent_solution, criterion, min_objective
):
    """
...
def check_incumbents(
    candidate_solution,
    incumbent_solution={},
    objective_measures=[DEFAULT_CRITERIA],
    objectives_min=[],
):
    """
    ...
    if not incumbent_solution:
        incumbent_solution = _create_incumbent(objective_measures)
    ...
    for criterion_index, criterion in enumerate(objective_measures):
        ...
        (new_incumbent, same_incumbent) = _compare_solutions_on_criteria(
            candidate_solution, incumbent_solution, criterion, min_objective
        )
...
def update_all_incumbents(candidate_solution, incumbent_solutions):
    ...
    incumbent_n_vehicles = update_number_of_vehicles_incumbent(
        candidate_solution, incumbent_n_vehicles
    )
    ...
    incumbent_cost_only = update_pure_total_cost_incumbent(
        candidate_solution, incumbent_cost_only
    )
