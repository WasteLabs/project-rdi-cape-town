...
class RouteSummary:
    ...
    def route_summary(self):
        ...
        solution_df_dist_travel = solution_df.loc[
            solution_df["activity_type"].isin(["travel"])
        ]
        solution_df_dist_collect = solution_df.loc[
            solution_df["activity_type"].isin(["collect"])
        ]

        solution_df_dist_travel = (
            solution_df_dist_travel.groupby(["route"])
            .agg(total_travel_dist=("length", "sum"))
            .reset_index()
        )
        solution_df_dist_collect = (
            solution_df_dist_collect.groupby(["route"])
            .agg(total_collect_dist=("length", "sum"))
            .reset_index()
        )

        solution_df = solution_df.loc[solution_df["activity_type"] != "travel"]
        activity_times = (
            solution_df.groupby(["route", "activity_type"])
            .agg(total_time=("activity_time", "sum"))
            .reset_index()
        )
        activity_times_offload = activity_times.loc[
            activity_times["activity_type"] == "offload"
        ].reset_index(drop=True)
        activity_times_collect = activity_times.loc[
            activity_times["activity_type"] == "collect"
        ].reset_index(drop=True)

        activity_times_offload = activity_times_offload.rename(
            columns={"total_time": "offload_time"}
        )
        activity_times_collect = activity_times_collect.rename(
            columns={"total_time": "collect_time"}
        )

        route_summary = (
            solution_df.groupby("route")
            .agg(
        ...
        route_summary["demand_collected"] = (
            route_summary["demand_collected"] / self.demand_conv
        )
        ...
        route_summary["total_travel_dist"] = (
            route_summary["total_travel_dist"] / self.dist_conv
        )
        route_summary["total_collect_dist"] = (
            route_summary["total_collect_dist"] / self.dist_conv
        )
        route_summary["total_dist"] = (
            route_summary["total_travel_dist"] + route_summary["total_collect_dist"]
        )
        route_summary["ev_consumption"] = (
            route_summary["total_travel_dist"] * self.ev_travel_demand
            + route_summary["total_collect_dist"] * self.ev_service_demand
            + route_summary["demand_collected"] * 1.5
        )
        route_summary["route"] = route_summary["route"] + 1

        if self.collection_schedule is not None:
            route_summary["Vehicle"] = self.collection_schedule["Vehicle"].values
            route_summary["Collection day"] = self.collection_schedule[
                "Collection day"
            ].values
        ...
        route_summary = route_summary.append(
            route_summary.sum(numeric_only=True), ignore_index=True
        )
        ...
        route_summary.loc[:, self.int_columns] = route_summary[self.int_columns].astype(
            int
        )
    ...
    def subtrip_summary(self):
        ...
        substrip_summary = (
            solution_df.groupby(["route", "subroute"])
            .agg(
                demand_collected=("activity_demand", "sum"),
        ...
        substrip_summary["activity_time"] = (
            substrip_summary["activity_time"] / self.cost_conv
        )
        substrip_summary["travel_time"] = (
            substrip_summary["travel_time"] / self.cost_conv
        )
        substrip_summary["total_time"] = (
            substrip_summary["travel_time"] + substrip_summary["activity_time"]
        )
    ...
    def route_trip_distance_summary_bar(self):
        ...
        solution_df = (
            solution_df.groupby(["route", "activity_type"])
            .agg(activity_distance=("length", "sum"))
            .reset_index()
        )
        solution_df["activity_distance"] = (
            solution_df["activity_distance"] / self.dist_conv
        )
        solution_df = solution_df.loc[
            solution_df["activity_type"].isin(["collect", "travel"])
        ]
    ...
    def clean_solution(self):
        ...
        # solution_df = solution_df.loc[solution_df['activity_type'] != 'depart_if']
        solution_df.loc[
            solution_df["activity_type"] == "offload",
            "total_traversal_time_to_activity",
        ] = np.nan
        solution_df["total_traversal_time_to_activity"] = solution_df[
            "total_traversal_time_to_activity"
        ].fillna(method="ffill")
    ...
    def geo_code_route(self):
        ...
        route_clean = route_clean.merge(
            self.network_info.main_arc_list,
            how="left",
    ...
    def plot_routes(self, figsize=None, linewidth_full=1, linewidth_req=2.5):
        ...
        _ = plot_solution.plot(
            ax=ax, linewidth=linewidth_req, column="route", legend=True
        )
        _ = self.offload.plot(ax=ax, markersize=50, facecolor="red")
        _ = self.depot.plot(ax=ax, markersize=50, facecolor="red")
        return fig
