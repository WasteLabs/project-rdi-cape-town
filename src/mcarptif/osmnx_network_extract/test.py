...
class NetworkExtract:
    ...
    def extend_required_inverse_arcs(self):
        ...
        self.reqEdgesPure = req_pure.loc[req_pure['req_arc_index'].isin(edges['req_arc_index'])]['req_arc_index'].values
        self.reqArcsPure = req_pure.loc[~req_pure['req_arc_index'].isin(edges['req_arc_index'])]['req_arc_index'].values
        ...
        edges_inv = edges_inv.merge(
            edges_index[['geom_id', 'arc_index_inv', 'req_arc_index_inv']],
            how='left')
        edges_inv.sort_values(['req_arc_index_inv'], inplace=True)
        edges_inv.reset_index(drop=True, inplace=True)
        edges_inv['req_arc_index'] = edges_inv.index + req_arcs[
            'req_arc_index'].max() + 1  # because indexing starts at zero
        ...
        req_inv_list_full = req_inv_list_full_df[
            ['arc_index', 'arc_index_inv', 'req_arc_index',
             'req_arc_index_inv']]
        req_inv_list_full = req_inv_list_full.where(
            pd.notnull(req_inv_list_full), None)
        ...
        reqArcListActual = req_inv_list_full.loc[~req_inv_list_full['req_arc_index'].isin(self.dummy_req_indices)]
        ...
        self.reqInvArcList = [int(x) if x is not None else None for x in req_inv_list_info]
    ...
    def create_main_arc_list(self):
        ...
        self.main_arc_list = self.main_arc_list.merge(self.network[['arc_index', 'u']], how='left')
        self.main_arc_list = self.main_arc_list.merge(self.network[['arc_index', 'v']], how='left')

        self.main_arc_list = self.main_arc_list.merge(self.nodes_u[['u', 'u_index']], how='left')
        self.main_arc_list = self.main_arc_list.merge(self.nodes_v[['v', 'v_index']], how='left')
    ...
    def load_distance_matrix(self):
        ...
        unique_req_arcs_v = temp_frame_df['v_index'].values  # this should be vertex-v of all arcs
        unique_req_arcs_u = temp_frame_df['u_index'].values  # this should be vertex-u of all arcs
    ...
    def extend_prop_info(self):
        ...
        all_index = network.loc[network['geom_id_order'].isin(prop_info_extended['geom_id_order'])][
    ...
    def add_solution(self, df_solution, extend=True, order=True):
        ...
        df_solution['total_traversal_time_to_activity'] = df_solution['total_traversal_time_to_activity'].fillna(
            method='ffill')
    ...
    def add_intermediate_hk_if(self,
                               intermediate_ifs,
                               pickup_return_calc,
                               bin_load=600,
                               bin_dump=600,
        ...
        offload_index = offload_index[['req_arc_index', 'arc_index', 'arc_index_inv',
                                       'req_arc_index_inv', 'orig_index']].merge(self.network[keep],
                                                                                 how='left')
        ...
        df_offload = df_offload.drop(columns=keep + ['arc_index_inv', 'req_arc_index_inv'])
    ...
    def deconstruct_solution(self):
        ...
        def create_sp(path):
            ...
            arc_frame['u_index-v_index'] = arc_frame['u_index'].astype(int).astype(str) + '-' + \
                                           arc_frame['v_index'].astype(int).astype(str)
            return arc_frame

        routes = self.df_solution.copy()
        routes['u_index-v_index'] = routes['u_index'].astype(int).astype(str) + '-' + routes['v_index'].astype(
            int).astype(str)
        ...
        network_plot['u_index-v_index'] = network_plot['u_index'].astype(int).astype(str) + '-' + network_plot[
            'v_index'].astype(int).astype(str)

        travel_paths = pd.concat(filler_frames)

        travel_paths = travel_paths.merge(
            network_plot[['u_index-v_index', 'geometry', 'length', 'maxspeed', 'u', 'v', 'arc_index']], how='left')
        travel_paths.reset_index(inplace=True, drop=True)
        travel_paths.sort_values(['length'], inplace=True)
        travel_paths.drop_duplicates(subset=['solution_index', 'u_index-v_index'], inplace=True)
        travel_paths.sort_values(['solution_index'], inplace=True)

        routes = routes.merge(network_plot[['arc_index', 'geometry', 'length', 'maxspeed']], how='left')
    ...
    def calc_bin_center_demand_fancy(self, prod, units=1):
        ...
        prod[[weight_col1_link, weight_col2_link, weight_col3_link]] = prod[
            [weight_col1_link, weight_col2_link, weight_col3_link]].fillna(0)
    ...
    def store_results(self,
        ...
        if self.producer_collection_schedule is not None:
            self.producer_collection_schedule.to_csv(
                'scenario_collection_schedule/{} - Collection schedule.csv'.format(scenario_name), index=False)
