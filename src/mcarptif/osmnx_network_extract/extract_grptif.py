...
class NetworkExtract:
    ...
    def __init__(
        ...
        Arg:
            ...
            network_directed (df): undirected arc network (one edge)
            nodes (df): nodes of network, used for SP calcs
        ...
        self.network_directed = network_directed.copy()
        self.nodes = nodes.copy()

        with tb.open_file(arc_h5_path, "r") as h5file:
            sp_info = h5file.root.shortest_path_info
            node_id_order = sp_info.node_id_order[:]
        node_id_order_df = pd.DataFrame({"u": node_id_order})
        node_id_order_df["sp_index"] = node_id_order_df.index
        self.nodes_order = node_id_order_df
        self.nodes = self.nodes.drop(columns="sp_index").merge(
            node_id_order_df, how="left", validate="1:1"
        )
        # TODO: the node index is not the default for the SP calcs, it's based on the edges
        self.nodes_u = self.nodes.copy()
        self.nodes_u.rename(columns={"u": "u", "sp_index": "u_index"}, inplace=True)

        self.nodes_v = self.nodes.copy()
        self.nodes_v.rename(columns={"u": "v", "sp_index": "v_index"}, inplace=True)

        self.network = self.network.merge(self.nodes_u[["u", "u_index"]])
        self.network = self.network.merge(self.nodes_v[["v", "v_index"]])
        ...
        self.req_inv_list_full_df = None
    ...
    def create_inv_list(self):
        ...
        #The step is no-longer necessary since we already have the inverse arc list.
        """
        ...
        network_edges = self.network_directed.copy()
        network = self.network.copy()
        circle = network_edges["u"] == network_edges["v"]
        edges_flag = (network_edges["oneway"] == False) & (circle == False)

        network_edges = network_edges.loc[~circle]
        edges = network_edges.dropna(subset=["geom_id_inv"])
        edges_inv = network.loc[network["geom_id"].isin(edges["geom_id_inv"])]

        edges_inv = network.copy().loc[edges_flag][
            ["u", "v", "oneway", "arc_index", "arc_id_ordered"]
        ]
        edges_key = network.copy().loc[edges_flag][
            ["u", "v", "oneway", "arc_index", "arc_id", "arc_id_ordered"]
        ]
        edges_inv = create_arc_id(edges_inv, "arc_id", "v", "u")
        logging.info("Number of edges: {}".format(edges_inv.shape[0]))

        edges_list = pd.merge(
            edges_key[["arc_index", "arc_id"]],
        ...
        inv_list_full = pd.concat([edges_list, arcs_index]).sort_values(["arc_index"])
    ...
    def load_required_arcs(self, required_arcs, merge_network=True):
        ...
        if required_arcs["arc_index"].isin(self.network["arc_index"]).all() is False:
        ...
        if required_arcs.duplicated(["geom_id_order"]).any() is True:
            logging.warning(
                "Required network contains both sides of edges, "
    ...
    def set_depot_arc(self, depot_arc_index):
        ...
        self.df_depot = (
            self.network.loc[self.network["arc_index"] == depot_arc_index]
            .iloc[[0]]
            .copy()
        )
    ...
    def set_if_arcs(self, if_arc_index):
        ...
        self.df_if = self.network.loc[
            self.network["arc_index"].isin(if_arc_index)
        ].copy()
    ...
    def loc_in_required_arcs(self):
        ...
        self.df_required_arcs_full = pd.concat(
            [self.df_depot, self.df_if, self.df_required_arcs]
        )
        self.df_required_arcs_full = self.df_required_arcs_full.reset_index(drop=True)
        self.df_required_arcs_full["req_arc_index"] = self.df_required_arcs_full.index
    ...
    def extend_required_inverse_arcs(self):
        ...
        network = self.network.copy()
        req_arcs = self.df_required_arcs_full.copy()

        dummy_indices = self.dummy_req_indices
        req_pure = req_arcs.loc[~req_arcs["req_arc_index"].isin(dummy_indices)].copy()

        edges = req_pure.dropna(subset=["geom_id_inv"])
        edges = edges.loc[edges["u"] != edges["v"]].copy()

        self.reqEdgesPure = req_pure.loc[
            req_pure["req_arc_index"].isin(edges["req_arc_index"])
        ]["req_arc_index"].values
        self.reqArcsPure = req_pure.loc[
            ~req_pure["req_arc_index"].isin(edges["req_arc_index"])
        ]["req_arc_index"].values

        edges_inv = network[network["geom_id"].isin(edges["geom_id_inv"])].copy()

        edges_index = edges.copy()
        edges_index["geom_id"] = edges_index["geom_id_inv"]
        edges_index["arc_index_inv"] = edges_index["arc_index"]
        edges_index["req_arc_index_inv"] = edges_index["req_arc_index"]

        edges_inv = edges_inv.merge(
            edges_index[["geom_id", "arc_index_inv", "req_arc_index_inv"]], how="left"
        )
        edges_inv.sort_values(["req_arc_index_inv"], inplace=True)
        edges_inv.reset_index(drop=True, inplace=True)
        edges_inv["req_arc_index"] = (
            edges_inv.index + req_arcs["req_arc_index"].max() + 1
        )  # because indexing starts at zero

        edges_inv_index = edges_inv.copy()
        edges_inv_index["geom_id"] = edges_inv["geom_id_inv"]
        edges_inv_index["arc_index_inv"] = edges_inv["arc_index"]
        edges_inv_index["req_arc_index_inv"] = edges_inv["req_arc_index"]

        req_arcs = req_arcs.merge(
            edges_inv_index[["geom_id", "arc_index_inv", "req_arc_index_inv"]],
            how="left",
        )

        req_inv_list_full_df = pd.concat([req_arcs, edges_inv])
        self.req_inv_list_full_df = req_inv_list_full_df.copy()

        req_inv_list_full = req_inv_list_full_df[
            ["arc_index", "arc_index_inv", "req_arc_index", "req_arc_index_inv"]
        ]
        req_inv_list_full = req_inv_list_full.where(pd.notnull(req_inv_list_full), None)
        req_inv_list_full = req_inv_list_full.replace({np.nan: None})

        req_inv_list_full.reset_index(drop=True, inplace=True)

        self.req_inv_list_full = req_inv_list_full.copy()
        self.reqArcList = req_inv_list_full["arc_index"].copy().values

        reqArcListActual = req_inv_list_full.loc[
            ~req_inv_list_full["req_arc_index"].isin(self.dummy_req_indices)
        ]
        reqArcListActual = reqArcListActual["req_arc_index"].copy().values
        self.reqArcListActual = reqArcListActual

        req_inv_list_info = req_inv_list_full["req_arc_index_inv"].values.copy()
        self.reqInvArcList = [
            int(x) if x is not None else None for x in req_inv_list_info
        ]
    ...
    def create_main_arc_list(self):
        ...
        self.main_arc_list.loc[self.reqEdgesPure, "arc_category"] = "required_edge"
        self.main_arc_list.loc[self.reqArcsPure, "arc_category"] = "required_arc"

        # Add node `u` and `v` for SP-calcs
        self.main_arc_list = self.main_arc_list.merge(
            self.network[["arc_index", "u"]], how="left"
        )
        self.main_arc_list = self.main_arc_list.merge(
            self.network[["arc_index", "v"]], how="left"
        )

        self.main_arc_list = self.main_arc_list.merge(
            self.nodes_u[["u", "u_index"]], how="left"
        )
        self.main_arc_list = self.main_arc_list.merge(
            self.nodes_v[["v", "v_index"]], how="left"
        )

    def check_main_list(self):
        """Few basic error checks on the main list"""
        logging.info("Checking master list")
        depot = self.main_arc_list.loc[self.main_arc_list["arc_category"] == "depot"]
        ifs = self.main_arc_list.loc[self.main_arc_list["arc_category"] == "offload"]
        arcs = self.main_arc_list.loc[
            ~self.main_arc_list["arc_category"].isin(["depot", "offload"])
        ]
        if depot.shape[0] > 1:
    ...
    def load_distance_matrix(self):
        ...
        temp_frame_df = self.main_arc_list

        n_arcs = temp_frame_df.shape[0]
        logging.info("Load distance matrix: {} x {}".format(n_arcs, n_arcs))

        req_arcs_v = temp_frame_df[["v_index"]]
        req_arcs_v_temp = req_arcs_v.copy().drop_duplicates(["v_index"])
        req_arcs_v_temp["index"] = req_arcs_v_temp.reset_index().index
        req_arcs_v = pd.merge(temp_frame_df, req_arcs_v_temp, how="left")
        req_arcs_v_unique = req_arcs_v_temp["v_index"].values
        redraw_v = req_arcs_v["index"].values

        req_arcs_u = temp_frame_df[["u_index"]]
        req_arcs_u_temp = req_arcs_u.copy().drop_duplicates(["u_index"])
        req_arcs_u_temp["index"] = req_arcs_u_temp.reset_index().index
        req_arcs_u = pd.merge(temp_frame_df, req_arcs_u_temp, how="left")
        req_arcs_u_unique = req_arcs_u_temp["u_index"].values
        redraw_u = req_arcs_u["index"].values

        req_arcs_v = temp_frame_df[["u_index"]]
        temp_frame2 = temp_frame_df.copy().drop_duplicates(["req_arc_index"])
        temp_frame2["index"] = temp_frame2.reset_index().index
        temp_frame = pd.merge(temp_frame_df, temp_frame2, how="left")
        # unique_req_arcs = temp_frame2['reqArcList'].values
        # TODO: fix it so that it works from v to u
        unique_req_arcs_v = temp_frame_df[
            "v_index"
        ].values  # this should be vertex-v of all arcs
        unique_req_arcs_u = temp_frame_df[
            "u_index"
        ].values  # this should be vertex-u of all arcs
        redraw = temp_frame["index"].values

        with tb.open_file(self.arc_h5_path, "r") as h5file:
            sp_info = h5file.root.shortest_path_info
            d_np_req = sp_info.cost_matrix[req_arcs_v_unique, :]
            d_np_req = d_np_req[:, req_arcs_u_unique]
            d_np_req = d_np_req[redraw_v, :]
            d_np_req = d_np_req[:, redraw_u]
    ...
    def offload_calculations(self):
        ...
        logging.info(
            "Calculate offloads: {} x {} x {}".format(n_arcs, n_arcs, self.n_ifs)
        )
        ...
        for k_if in self.IFarcsnewkey:
            for i in range(n_arcs):
                for j in range(n_arcs):
                    visitT = self.d[i][k_if] + self.d[k_if][j]
    ...
    def set_inverse_values(self, df_value, col):
        ...
        if not df_value["arc_index"].isin(self.df_required_arcs["arc_index"]).all():
            logging.error(
                "Costs are included for arcs that are not required " "to service."
            )
            raise ValueError

        if not self.df_required_arcs["arc_index"].isin(df_value["arc_index"]).all():
            logging.error("Not all required arcs have been assigned a " "service cost")
            raise ValueError
        ...
        inv_arc_cost_full = inv_arc_cost_full.merge(
            df_value[["arc_index_inv", col_inv]], how="left"
        )
    ...
    def set_service_cost(self, df_service_cost):
        ...
        self.df_service_cost = self.set_inverse_values(
            df_service_cost, "service_cost"
        ).copy()
    ...
    def set_service_demand(self, df_demand):
        ...
        self.df_demand = self.set_inverse_values(df_demand, "demand")
    ...
    def calc_min_duration_capacity(self):
        ...
        self.min_duration = (
            self.d[0, req_arcs] + service[req_arcs] + self.if_cost_np[req_arcs, 0]
        )
    ...
    def set_vechile_capacity_constraint(self, capacity):
        ...
        if capacity < self.min_capacity:
            logging.warning("Vehicle capacity limit is too low at {}".format(capacity))
            logging.warning(
                "Highest collection point demand is {}".format(self.min_capacity)
            )
    ...
    def set_vehicle_duration_constraint(self, duration):
        ...
        if duration < self.min_duration:
            logging.warning(
                "Vehicle duration limit is too short at {}".format(duration)
            )
            logging.warning(
                "Longest collection point time, including travel "
    ...
    def extend_prop_info(self):
        ...
        prop_info_extended = pd.merge(
            prop_info_extended,
            network[["arc_index", "geom_id_order"]],
            how="left",
            left_on="arc_index",
            right_on="arc_index",
        )

        all_index = network.loc[
            network["geom_id_order"].isin(prop_info_extended["geom_id_order"])
        ][["arc_index", "geom_id_order"]]
        prop_info_extended = prop_info_extended.drop(columns=["arc_index"])
        prop_info_extended = pd.merge(
            prop_info_extended,
            all_index,
            how="left",
            left_on="geom_id_order",
            right_on="geom_id_order",
        )
    ...
    def network_gdf(self, producers=None):
        ...
        if producers is None:
            self.df_required_arcs_plot = create_gdf(
                self.df_required_arcs, dropna_geom=True
            )
        else:
            self.df_required_arcs_plot = create_latlon_gdf(producers, dropna_geom=True)

    def plot_required_arcs(self, figsize=None, linewidth_full=1, linewidth_req=2.5):
        if figsize is None:
            figsize = (40, 40)

        fig, ax = plt.subplots(figsize=figsize)
        _ = self.network_plot.plot(ax=ax, linewidth=linewidth_full)
        _ = self.df_required_arcs_plot.plot(
            ax=ax, markersize=linewidth_req, color="red"
        )
        return fig
    ...
    def add_solution(self, df_solution, extend=True, order=True):
        ...
        df_solution = df_solution.rename(columns={"activity_id": "req_arc_index"})
        ...
        df_solution.loc[
            df_solution["activity_type"] == "offload",
            "total_traversal_time_to_activity",
        ] = np.nan
        df_solution["total_traversal_time_to_activity"] = df_solution[
            "total_traversal_time_to_activity"
        ].fillna(method="ffill")
        ...
        if order:
            ...
            total_time = (
                df_solution.groupby(["route"])
                .agg(total_time=("cum_time", "max"))
                .reset_index()
            )
            ...
            df_solution = df_solution.sort_values(
                ["total_time", "temp_index"], ascending=False
            )
            df_solution["route"] = df_solution["new_route"]
            df_solution = df_solution.drop(
                columns=["temp_index", "total_time", "new_route"]
            )
    ...
    def add_intermediate_hk_if(
        ...
        solution_collect = solution_df_temp.loc[
            solution_df_temp["activity_type"] == "collect"
        ].copy()
        solution_df_temp = solution_df_temp.loc[
            solution_df_temp["activity_type"].str.contains("depot", regex=False)
        ]

        solution_df_temp["activity_time"] = 0

        solution_collect["activity_time"] = bin_load
        solution_collect["total_traversal_time_to_activity"] = 0

        offload_index = self.df_solution.merge(
            pickup_return_calc.req_arcs[["req_arc_index", "min_offload_arc"]]
        )
        offload_index = offload_index[["req_arc_index", "min_offload_arc"]]
        offload_index["orig_index"] = offload_index["req_arc_index"]
        offload_index["req_arc_index"] = offload_index["min_offload_arc"]
        offload_index = offload_index.merge(pickup_return_calc.offloads)
        keep = ["arc_index", "u", "v", "u_index", "v_index", "geom_id_order"]
        ...
        ].merge(self.network[keep], how="left")
        offload_index["req_arc_index"] = offload_index["orig_index"]
        offload_index = offload_index.drop(columns=["orig_index"])

        df_offload = solution_collect.copy()
        # df_offload['req_arc_index'] = if_inter
        df_offload["index_temp"] = df_offload["index_temp"] + 0.1
        df_offload["activity_type"] = "offload"
        df_offload["activity_time"] = bin_dump
        df_offload["arc_category"] = "offload"
        df_offload["activity_demand"] = 0
        df_offload["total_traversal_time_to_activity"] = 0

        df_offload[["n_bins", "n_units", "speed"]] = np.nan
        df_offload = df_offload.drop(
            columns=keep + ["arc_index_inv", "req_arc_index_inv"]
        )

        df_offload = df_offload.merge(offload_index)
        # df_offload[['arc_index_inv', 'req_arc_index_inv']] = -1

        print(df_offload.shape)
        ...
        df_return["activity_time"] = bin_offload
        df_return["total_traversal_time_to_activity"] = 0
        # df_return[['n_bins', 'n_units', 'speed', 'geom_id_order', 'req_arc_index', 'arc_index']] = np.nan
    ...
    def deconstruct_solution(self):
        ...
        def create_sp(path):
            u = node_list.iloc[path[:-1]].index
            v = node_list.iloc[path[1:]].index
            arc_frame = pd.DataFrame({"u_index": u, "v_index": v})
            arc_frame["u_index-v_index"] = (
                arc_frame["u_index"].astype(int).astype(str)
                + "-"
                + arc_frame["v_index"].astype(int).astype(str)
            )
            return arc_frame

        routes = self.df_solution.copy()
        routes["u_index-v_index"] = (
            routes["u_index"].astype(int).astype(str)
            + "-"
            + routes["v_index"].astype(int).astype(str)
        )
        routes["solution_index"] = routes.index
        frame_index = routes["solution_index"].values
        route_number = routes["route"].values
        arc_index_v = routes["v_index"].values
        arc_index_u = routes["u_index"].values
        ...
        with tb.open_file(self.arc_h5_path, "r") as h5file:
            ...
            for i in range(0, n_solution_arcs - 1):
                arc_i = arc_index_v[i]
                arc_j = arc_index_u[i + 1]
                frame_i = frame_index[i]

                if route_number[i] == route_number[i + 1]:
                    sp_path = sp_full(p_full, arc_i, arc_j, full=True)
                    n_arcs = len(sp_path) - 1
                    if n_arcs > 0:
                        new_index = frame_i + np.array(range(1, n_arcs + 1)) / (
                            n_arcs + 1
                        )

                        temp_frame = create_sp(sp_path)
                        temp_frame["solution_index"] = new_index
        ...
        network_plot = self.network.copy()
        network_plot["u_index-v_index"] = (
            network_plot["u_index"].astype(int).astype(str)
            + "-"
            + network_plot["v_index"].astype(int).astype(str)
        )

        travel_paths = pd.concat(filler_frames)

        travel_paths = travel_paths.merge(
            network_plot[
                [
                    "u_index-v_index",
                    "geometry",
        ...
        travel_paths.reset_index(inplace=True, drop=True)
        travel_paths.sort_values(["length"], inplace=True)
        travel_paths.drop_duplicates(
            subset=["solution_index", "u_index-v_index"], inplace=True
        )
        travel_paths.sort_values(["solution_index"], inplace=True)

        routes = routes.merge(
            network_plot[["arc_index", "geometry", "length", "maxspeed"]], how="left"
        )

        routes_full = pd.concat([routes, travel_paths])
        routes_full = routes_full.sort_values(["solution_index"])
        routes_full[["route", "subroute"]] = routes_full[["route", "subroute"]].fillna(
            method="ffill"
        )
        routes_full = routes_full.reset_index()
        routes_full = routes_full.drop(columns="solution_index")
        # routes_full = routes_full.merge(network_plot[['u_index-v_index', 'geometry', 'length', 'maxspeed']], how='left')

        # = create_gdf(routes_full, dropna_geom=True)
    ...
    def plot_routes(
        ...
        df_solution_map["Route"] = df_solution_map["Route"].apply(
            lambda x: "0" + x if len(x) == 1 else x
        )
        df_solution_map["Route"] = "Route " + df_solution_map["Route"].astype(str)

        df_solution_map_travel = df_solution_map.loc[
            df_solution_map["activity_type"] == "travel"
        ]
        df_solution_map_collect = df_solution_map.loc[
            df_solution_map["activity_type"] != "travel"
        ]

        fig, ax = plt.subplots(figsize=figsize)
        # _ = self.network_plot.plot(ax=ax, linewidth=linewidth_full, color='grey')
        _ = df_solution_map_travel.plot(
            ax=ax, linewidth=linewidth_full, column="Route", legend=True, ls="dotted"
        )
        _ = df_solution_map_collect.plot(
            ax=ax, linewidth=linewidth_req, column="Route", legend=True
        )
        if plot_customers:
            _ = self.df_required_arcs_plot.plot(
                ax=ax, markersize=linewidth_req_cust, color="red"
            )
    ...
    def add_time_formatted(
        ...
        if tz:
            df["time"] = (
                df["time"].dt.tz_localize(None).dt.tz_localize("Asia/Singapore")
            )
        df["time"] = df["time"] + pd.to_timedelta(df[t_col], unit=t_units)
        self.df_solution_full = df

    def add_constant_duration_time(
        self, start_time="2021-07-01 07:00:00", duration=30, t_units="sec"
    ):
        ...
        df["time_const_dur"] = df["time_const_dur"] + pd.to_timedelta(
            df["duration"], unit=t_units
        )
        self.df_solution_full = df
    ...
    def arc_consolidation_standard(self, prod):
        ...
        prop_info = (
            prod.groupby(["arc_index"])
            .agg(
                n_bins=("Postal Code", "count"),
        ...
        prop_info = prop_info.merge(
            self.network[["arc_index", "length", "maxspeed"]], how="left"
        )
        prop_info = prop_info.rename(
            columns={"length": "distance", "maxspeed": "speed"}
        )
    ...
    def bin_center_consolidation(self, prod):
        ...
        prop_info = (
            ...
            .agg(
                ...
                n_units=("Total units", "sum"),
                demand=("Calculated weight", "sum"),
            )
            .reset_index()
        )

        self.prop_info = prop_info.copy()
        self.prop_info = self.prop_info.merge(
            self.network[["arc_index", "length", "maxspeed"]], how="left"
        )
        self.prop_info = self.prop_info.rename(
            columns={"length": "distance", "maxspeed": "speed"}
        )
        return self.prop_info
    ...
    def calc_bin_center_demand_fancy(self, prod, units=1):
        ...
        prod[[weight_col1_link, weight_col2_link, weight_col3_link]] = prod[
            [weight_col1_link, weight_col2_link, weight_col3_link]
        ].fillna(0)

        prod["demand"] = (
            prod[weight_col1] * prod[weight_col1_link]
    ...
    def add_demand_service_cost_to_ars(self, df, speed=10 / 3.6):
        df_arc = (
            df.groupby(["arc_index"])
            .agg(demand=("demand", "sum"), bin_service_cost=("bin_service_cost", "sum"))
            .reset_index()
        )
        df_arc = df_arc.merge(self.network[["arc_index", "length"]])
        df_arc["service_travel_cost"] = df_arc["length"] / speed
        df_arc["service_cost"] = (
            df_arc["bin_service_cost"] + df_arc["service_travel_cost"]
        )
        return df_arc
    ...
    def calc_bin_center_service_fancey(self, prod, units=60):
        ...
        prod["service_cost"] = (
            prod[service_col1_link] / prod[service_col1] + prod[service_col2]
        )
    ...
    def vehicle_collection_schedule(self, schedule="weekly"):
        ...
        for day in dow_collection:
            ...
            if route_count == n_routes:
                break
        self.collection_schedule = pd.DataFrame(
            {"Vehicle": vehicle_assigned, "Collection day": schedule_route}
        )
    ...
    def add_route_colors(self):
        ...
        # self.df_solution_full = color_df(
        #     self.df_solution_full, group="route", opacity=0.5
        # )
    ...
    def extract_wide_offload_totals(self):
        ...
        activity_df["cum_demand"] = activity_df["cum_demand"].fillna(method="ffill")
        df_offloads = activity_df.loc[offloads]
        df_offloads = df_offloads[
            ["route", "subroute", "arc_index", "cum_demand", "cum_time"]
        ]
        poi_locations = key_pois[["description", "arc_index"]].drop_duplicates()
        df_offloads = df_offloads.merge(poi_locations, how="left")
        df_offloads = (
            df_offloads.groupby(["route", "description"])
            .agg(total_disposed=("cum_demand", "sum"))
            .reset_index()
        )
        df_offloads["total_disposed"] = (df_offloads["total_disposed"] / 1000).round(2)

        offload_sum = df_offloads.pivot_table(
            index=["route"], columns="description", values=["total_disposed"]
        )
        offload_sum.columns = [
            f"Tons disposed at @ {y}" for x, y in offload_sum.columns
        ]
    ...
    def full_producer_report(self, bc=False, bc_full=False):
        ...
        if bc_full:
            df_arc_producer = df_arc_producer.loc[
                df_arc_producer["activity_type"] != "offload"
            ]
        ...
        df_arc_producer["Service time window"] = (
            df_arc_producer["time_start"].astype(str).str[10:]
            + " - "
            + df_arc_producer["time_end"].astype(str).str[10:]
        )
        df_arc_producer = df_arc_producer.loc[
            df_arc_producer["activity_type"] != "travel"
        ]
        df_inv_arcs = df_arc_producer["arc_category"] == "required_edge_inverse"
        df_arc_producer.loc[df_inv_arcs, "arc_index"] = df_arc_producer["arc_index_inv"]
        ...
        df_arc_producer = df_arc_producer.merge(self.collection_schedule)
        ...
        route_kpi_add = route_kpi.merge(
            pois, left_on="arc_index", right_on="arc_index", how="left"
        )
        ...
        if not bc:
            ...
            route_kpi_add["Service time window"] = (
                route_kpi_add["type"] + " " + route_kpi_add["time"].astype(str).str[10:]
            )
        ...
        df_arc_final = pd.concat(
            [df_arc_producer, route_kpi_add], ignore_index=True
        ).sort_values(["index_temp"])
        df_arc_final = df_arc_final.reset_index(drop=True)
        df_arc_final = df_arc_final.drop(columns=["index_temp"])

        if not bc:
            df_arc_final[["demand", "total units", "total_people"]] = df_arc_final[
                ["demand", "total units", "total_people"]
            ].fillna(0)
            df_arc_final["demand"] = df_arc_final["demand"].round(2)
            df_arc_final[["route", "total units", "total_people"]] = df_arc_final[
                ["route", "total units", "total_people"]
            ].astype(int)
            df_arc_final["route"] = df_arc_final["route"] + 1
            df_arc_final.columns = [
                "Vehicle",
                "Route",
                "Collection day",
                "Service time window",
            ...
            df_arc_final[["demand", "Total units", "No of Blks Served"]] = df_arc_final[
                ["demand", "Total units", "No of Blks Served"]
            ].fillna(0)
            df_arc_final["demand"] = df_arc_final["demand"].round(2)
            df_arc_final[
                ["route", "demand", "Total units", "No of Blks Served"]
            ] = df_arc_final[
                ["route", "demand", "Total units", "No of Blks Served"]
            ].astype(
                int
    ...
    def store_results(self, scenario_name, prod_select, route_sum_table, r):
        self.df_solution_full.to_csv(
            "scenario_full_routes/{} - Collection routes.csv".format(scenario_name),
            index=False,
        )
        prod_select.to_csv(
            "scenario_producers/{} - Full service points.csv".format(scenario_name),
            index=False,
        )
        ...
        if self.producer_collection_schedule is not None:
            self.producer_collection_schedule.to_csv(
                "scenario_collection_schedule/{} - Collection schedule.csv".format(
                    scenario_name
                ),
                index=False,
            )
        ...
        r.to_html(
            filename="scenario_maps_html/{} - Collection maps.html".format(
                scenario_name
            ),
            offline=True,
    ...
    def write_KIP_to_excel(self, route_sum_table, scenario_name):
        ...
        writer = pd.ExcelWriter(
            "scenario_KPIs_xlsx/{} - KPIs.xlsx".format(scenario_name),
            engine="xlsxwriter",
        )
