...
class NetworkExtract:
    """Extract all VRP network info in the format required for the MCARPTIF.

    As rules:
        * depot is always the first entry in the required arc list
        * thereafter, intermediate facilities
        * thereafter, required nodes, to be interpreted as directed arcs.
    ...
    def __init__(
        self, network, arc_h5_path, round_cost=True, length_int=True, key_pois=None
    ):
        """
        Arg:
    ...
    def extend_required_inverse_arcs(self):
        ...
        req_arcs_pure = self.df_required_arcs_full.loc[
            self.df_required_arcs_full["arc_type"] == "collection"
        ].copy()
        ...
        req_inv_list = pd.DataFrame(
            {
                "arc_index": full_list,
                "arc_index_inv": self.df_inv_list.iloc[full_list][
                    "arc_index_inv"
                ].values,
            }
        )
        ...
        dummy_index = np.concatenate([[self.depot_arc_index], self.if_arc_index])
        dummy_req_arc_index = self.dummy_req_indices

        dummy_inv_list = pd.DataFrame(
            {"arc_index": dummy_index, "req_arc_index": dummy_req_arc_index}
        )

        req_inv_list_full = pd.concat([dummy_inv_list, req_inv_list])

        req_inv_list_full = req_inv_list_full.replace({np.nan: None})
        req_inv_list_full = req_inv_list_full[
        ...
        orig = req_inv_list.loc[
            req_inv_list["arc_index"].isin(self.df_required_arc_index_orig)
        ]
    ...
    def update_cost_matrix_circular(self):
        logging.info("Update travel durations.")
        self.d = self.d_np_req_orig.copy() / self.travel_speed
        self.d = self.d * 2
        ...
        self.d[self.depot_arc_index, :] = (
            self.d_np_req_orig[self.depot_arc_index, :].copy() / self.travel_speed
        )
        self.d[:, self.depot_arc_index] = (
            self.d_np_req_orig[:, self.depot_arc_index].copy() / self.travel_speed
        )
        self.d[self.if_arc_index, :] = (
            self.d_np_req_orig[self.if_arc_index, :].copy() / self.travel_speed
        )
        self.d[:, self.if_arc_index] = (
            self.d_np_req_orig[:, self.if_arc_index].copy() / self.travel_speed
        )
    ...
    def extend_prop_info(self):
        ...
        all_index = network.loc[
            network["arc_id_ordered"].isin(prop_info_extended["arc_id_ordered"])
    ...
    def deconstruct_solution(self, node_list, arc_list):
        """Fill shortest paths arc indices into solution and assign arc
        categories."""

        def find_sp(p, start, end):
            path = [end]
            pred = end
            line_search = p[start, :]
            while pred > 0 and pred != start:
                pred = line_search[path[0]]
                path.insert(0, pred)
            return path

        def sp_path_to_arcs(sp_path):
            u = node_list.iloc[sp_path[:-1]]["u"].values
            v = node_list.iloc[sp_path[1:]]["u"].values
            arc_frame = pd.DataFrame({"u": u, "v": v})
            arc_frame["u-v"] = (
                arc_frame["u"].astype(int).astype(str)
                + "-"
                + arc_frame["v"].astype(int).astype(str)
            )
            return arc_frame[["u-v"]]
        ...
        routes = routes.merge(node_list[["arc_index", "geometry"]], how="left")
        ...
        with tb.open_file(self.arc_h5_path, "r") as h5file:
            ...
            for i in range(0, n_solution_arcs - 1):
                ...
                if route_number[i] == route_number[i + 1]:
                    sp_path = find_sp(p_full, arc_i, arc_j)
                    arcs = sp_path_to_arcs(sp_path).copy()
                    n_arcs = arcs.shape[0]
                    if n_arcs > 0:
                        new_index = frame_i + np.array(range(1, n_arcs + 1)) / (
                            n_arcs + 1
                        )
                        arcs["solution_index"] = new_index
                        arcs["arc_category"] = "travel"
                        arcs["activity_type"] = "travel"
                        arcs["solution_group"] = frame_i
                        filler_frames.append(arcs)

        travel_paths = pd.concat(filler_frames)
        travel_paths = travel_paths.merge(
            arc_list[["geometry", "length", "u-v", "maxspeed"]],
            how="left",
            left_on="u-v",
            right_on="u-v",
        )

        travel_paths = travel_paths.sort_values(["length"])

        travel_paths = travel_paths.drop_duplicates(
            ["solution_group", "u-v"], keep="first"
        )
        travel_paths = travel_paths.sort_values(["solution_index"])

        # travel_paths = travel_paths.drop(columns=['u-v'])
    ...
    def full_producer_report(self, bc=False, bc_full=False):
        ...
        self.producer_collection_schedule = self.producer_collection_schedule.dropna(
            subset=["Name"]
        )
    ...
    def store_results(
        ...
        path_create("scenario_full_routes/")
        path_create("scenario_producers/")
        path_create("scenario_producers_full/")
        path_create("scenario_collection_schedule/")
        path_create("scenario_maps_html/")
        path_create("scenario_KPIs/")
        ...
        prod_select.to_csv(
            "scenario_producers_full/{} - Full service points.csv".format(
                scenario_name
            ),
            index=False,
        )
        ...
        r.save_to_html(
            file_name="scenario_maps_html/{} - Collection maps.html".format(
                scenario_name
            ),
            data=data_out,
            read_only=True,
