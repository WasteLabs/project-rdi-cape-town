...
Arc-geometry is used as the arc-key:

'geom_id', 'geom_id_inv', 'geom_id_order'
"""
import logging
...
def calc_dist(df, x0="x", y0="y", x1="x1", y1="y1"):
    dist = (
        ((df[x0].subtract(df[x1])).pow(2)).add((df[y0].subtract(df[y1])).pow(2))
    ).pow(0.5)
    return dist
...
def create_gdf(df, crs="EPSG:4326", dropna_geom=False):
    ...
    if dropna_geom is True:
        df = pd.DataFrame(df.dropna(subset=["geometry"]))
...
def customer_network_plot(customers, network, figsize=None, markersize=5, linewidth=1):
    if figsize is None:
        figsize = (40, 40)
...
def required_arc_plot(
    full_network, required_arcs, figsize=None, linewidth_full=1, linewidth_req=2.5
):
    if figsize is None:
        figsize = (40, 40)
...
class NetworkCode:
    ...
    def __init__(self, df_network=None, path=None, country="SG", xy_converted=True):
        """
        Arg:
        ...
        self.df_network = create_gdf(
            self.df_network, self.crs["latlon"], dropna_geom=True
        )
        ...
        self.arc_id = "arc_id"
        self.arc_id_ordered = "arc_id_ordered"

    def set_arc_ids(self, arc_id, arc_id_ordered):
        """Set arc IDs and ordered arc IDs if they are not `arc_id` and
        `arc_id_ordered.
        """
        self.arc_id = arc_id
        self.arc_id_ordered = arc_id_ordered
    ...
    def add_customers(self, df_points, gdf_convert=True):
        ...
        self.df = create_latlon_gdf(
            self.df, crs=self.crs["latlon"], x_col="lon", y_col="lat"
        )
        if gdf_convert:
    ...
    def create_nodes_gdf(self, df_network):
        ...
        df_nodes = extract_node_gdf(df_network, self.crs_ref)
        return df_nodes

    def convert_direct(self, arc_id_ordered_key=None, set_filter=True):
        ...
        """
        if arc_id_ordered_key is None:
            arc_id_ordered_key = self.arc_id_ordered
    ...
    def filter_network(self, filter_values, filter_column="highway"):
        ...
        for string in filter_values:
            contained = contained.copy() | network_col.str.contains(string, regex=False)

        self.df_network_filter = self.df_network.loc[contained]
        uv = np.append(self.df_network_filter["u"], self.df_network_filter["v"])
        uv = np.unique(uv)
    ...
    def assign_closest_edge_vertex(self, df):
        ...
        vertices_keep = (
            vertices[["u", "geometry"]].copy().rename(columns={"u": "closest_u"})
        )
    ...
    def snap_customers(self, df):
        ...
        def arc_snapping(df_row):
            arc = edges_df.loc[edges_df[arc_id] == df_row.name].geometry.unary_union
            points = df_row.geometry
            result = points.apply(lambda row: arc.interpolate(arc.project(row)))
    ...
    def find_best_collection(df):
        ...
        df.loc[arc_closest, "lon_collect"] = df.loc[arc_closest]["lon_arc_snap"]
        df.loc[arc_closest, "lat_collect"] = df.loc[arc_closest]["lat_arc_snap"]
        df.loc[~arc_closest, "lon_collect"] = df.loc[~arc_closest]["lon_u"]
        df.loc[~arc_closest, "lat_collect"] = df.loc[~arc_closest]["lat_u"]

        df.loc[arc_closest, "dist_collect"] = df.loc[arc_closest, "arc_dist"]
        df.loc[~arc_closest, "dist_collect"] = df.loc[~arc_closest, "dist_u"]
        return df

    def find_nearest_collection_point(
        self, dist=None, snap_arcs=True, key_merge=True, return_all_arcs=False
    ):
        ...
        logging.info(
            "Number of arcs {} number of points {}".format(
                network.shape[0], df_fit.shape[0]
            )
        )
        G = ox.graph_from_gdfs(self.df_nodes_filter, self.df_network_filter)
        # add hack to return all...
        if return_all_arcs is False:
            edge_assign = ox.distance.nearest_edges(
                G,
                df_fit[x_col],
                df_fit[y_col],
            ...
            edge_assign = self.df_network_filter.reset_index()[["u", "v", "key"]].values
            df_fit = df_fit.iloc[: len(edge_assign)]
            # N = np.ceil(len(df_fit) / len(edge_assign))
            # edge_assign = np.repeat(edge_assign, N, axis=0)[: len(df_fit)]

        edge_assign = np.array(edge_assign)
        u = edge_assign[:, 0]
        v = edge_assign[:, 1]
        k = edge_assign[:, 2]
        df_fit["arc_u"] = u
        ...
        if key_merge:
            df_fit["arc_key"] = k
            df_fit = create_arc_id(df_fit, self.arc_id, "arc_u", "arc_v", "arc_key")

        df_fit = self.assign_closest_edge_vertex(df_fit)
        network = network.rename(columns={"geometry": "geometry_arc"})

        df_fit = df_fit.merge(network[[self.arc_id, "geometry_arc"]], how="left")
        if snap_arcs:
    ...
    def merge_network(self):
        ...
        self.df_collection_points = self.df_collection_points.merge(
            self.df_network_filter[[self.arc_id, "highway", "oneway"]]
        )
    ...
    def convert_geometries_latlon(self, columns=None):
        ...
        if columns is None:
            columns = [
                x for x in self.df_collection_points.columns if x.find("geometry") != -1
            ]

        for col in columns:
            self.df_collection_points = self.df_collection_points.set_geometry(
                col, crs=self.crs["xy"]
            )
            self.df_collection_points = self.df_collection_points.to_crs(
                crs=self.crs["latlon"]
            )
...
def prepare_node_routing(node_list, edge_list):
    ...
    edge_list["u-v"] = (
        edge_list["u"].astype(int).astype(str)
        + "-"
        + edge_list["v"].astype(int).astype(str)
    )
    edge_list["arc_index"] = edge_list.index
    edge_list["arc_id"] = (
        edge_list["u"].astype(str)
        + "-"
        + edge_list["v"].astype(str)
        + "-"
        + edge_list["key"].astype(str)
    )
